...
def open_app(dependency,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        sTestStepReturnStatus = CustomScript.launch(package_name,activity_name)
...
def confirm_right_menu_items(dependency,step_data,file_attachment,temp_q):
    sModuleInfo = inspect.stack()[0][3] + " : " + inspect.getmoduleinfo(__file__).name
    try:
        sTestStepReturnStatus = CustomScript.confirm_right()
        print sTestStepReturnStatus
        temp_q.put(sTestStepReturnStatus)
        return sTestStepReturnStatus
    except Exception, e:
...
def go_to_a_left_menu_section(dependency,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        sTestStepReturnStatus = CustomScript.go_left(section_name)
        print sTestStepReturnStatus
        temp_q.put(sTestStepReturnStatus)
        return sTestStepReturnStatus
    except Exception, e:
...
def close_app(dependency,step_data,file_attachment,temp_q):
    sModuleInfo = inspect.stack()[0][3] + " : " + inspect.getmoduleinfo(__file__).name
    try:
        sTestStepReturnStatus = CustomScript.close()
        print sTestStepReturnStatus
        temp_q.put(sTestStepReturnStatus)
        return sTestStepReturnStatus
    except Exception, e:
...
def open_browser(dependency,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        browser_name=first_data_set[0][1]
...
def car_selection(dependency,step_data,file_attachment, temp_q):
    ...
    try:
        ...
        print car_data
        return "passed"
        # if CustomScript.select_base_car(car_data[0]):
        #     CommonUtil.ExecLog(sModuleInfo,"Selected the first car successfully",1,local_run)
        #     for index,e in enumerate(car_data[1:]):
        #         CustomScript.select_car(e,(index+1))
        #     return "passed"
        # else:
...
def verify_data(dependency,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        total_list=CustomScript.read_data_from_page(tag_list)
        oCompare=CompareModule()
        expected_list=c.make_single_data_set_compatible(rest_data)
        actual_list=c.make_single_data_set_compatible(total_list)
...
def lock_car(dependency,steps_data,file_attachment,temp_q):
    ...
    try:
        ...
        stepReturnStatus=CustomScript.lock_unlock_car(car_number,True)
...
def unlock_car(dependency,steps_data,file_attachment,temp_q):
    ...
    try:
        first_data_set=steps_data[0]
        car_number=int(first_data_set[0][1])
        stepReturnStatus=CustomScript.lock_unlock_car(car_number)
        temp_q.put(stepReturnStatus)
        return stepReturnStatus
    except Exception, e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
...
def open_detail_pop_up(dependency,steps_data,file_attachment,temp_q):
    ...
    try:
        ...
        actual_data=CustomScript.open_detail_pop_up(tag_to_click,description_to_match)
        if actual_data:
            oCompare=CompareModule()
            expected_list=c.make_single_data_set_compatible(first_data_set)
            actual_list=c.make_single_data_set_compatible(actual_data)
...
def check_nissan_advantage_tag(dependency,steps_data,file_attachment,temp_q):
    ...
    try:
        first_data_set=steps_data[0]
        stepReturnStatus=CustomScript.check_nissan_adv_tag(first_data_set[0][1])
        temp_q.put(stepReturnStatus)
        return stepReturnStatus
    except Exception, e:
        exc_type, exc_obj, exc_tb = sys.exc_info()

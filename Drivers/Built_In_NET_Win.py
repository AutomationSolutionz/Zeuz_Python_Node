...
def copy_attachments_to_sharepoint(
    dependency,
    run_time_params,
    step_data,
    file_attachment,
    ...
    try:
        sTestStepReturnStatus = NETAutomation.save_email_attachment_to_sharepoint(
            step_data
        )
...
def synchronize_all_emails_using_colligo(
    dependency,
    run_time_params,
    step_data,
    file_attachment,
    ...
    try:
        sTestStepReturnStatus = NETAutomation.synchronize_all_emails_using_colligo(
            step_data
        )
        return CommonUtil.Result_Analyzer(sTestStepReturnStatus, temp_q)
    except Exception:

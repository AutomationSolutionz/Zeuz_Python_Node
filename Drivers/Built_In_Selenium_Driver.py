...
def open_browser(dependency,run_params,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        if stepReturn == "passed":
            ...
            CommonUtil.ExecLog(sModuleInfo,"Failed to start the browser",1)
            temp_q.put(stepReturn)
            CommonUtil.ExecLog(sModuleInfo,"Exit: Step - Open Browser",1)
            return stepReturn
        else:
            CommonUtil.ExecLog(sModuleInfo,"Step return type unknown: %s" %(stepReturn),1)
            temp_q.put("failed")
            CommonUtil.ExecLog(sModuleInfo,"Exit: Step - Open Browser",1)
            return "failed"
    except Exception, e:
...
#         sTestStepReturnStatus=Selenium_Built_In.Go_To_Link(web_link, web_title)
        ...
        if sTestStepReturnStatus == "passed":
            ...
            CommonUtil.ExecLog(sModuleInfo,"Step return type unknown: %s" %(sTestStepReturnStatus),1)
            temp_q.put("failed")
            CommonUtil.ExecLog(sModuleInfo,"Exit: Step - Go To Webpage Selenium",1)
            return "failed"
    except Exception, e:
...
def keystroke_selenium(dependency,run_time_params,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        temp_q.put(sTestStepReturnStatus)
        CommonUtil.ExecLog(sModuleInfo,"Exit: Step - Keystroke_For_Element Selenium",1)
        return sTestStepReturnStatus
    except Exception, e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
def hover_over_selenium(dependency,run_time_params,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        temp_q.put(sTestStepReturnStatus)
        CommonUtil.ExecLog(sModuleInfo,"Exit: Step - Hover_Over_Element Selenium",1)
        return sTestStepReturnStatus
    except Exception, e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
def wait_for_element_selenium(dependency,run_time_params,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        temp_q.put(sTestStepReturnStatus)
        CommonUtil.ExecLog(sModuleInfo,"Exit: Step - Wait for Element to appear Selenium",1)
        return sTestStepReturnStatus
    except Exception, e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
def validate_table_selenium(dependency,run_time_params,step_data,file_attachment,temp_q):
    ...
    try:
        ...
        temp_q.put(sTestStepReturnStatus)
        CommonUtil.ExecLog(sModuleInfo,"Exit: Step - Validate Table",1)
        return sTestStepReturnStatus
    except Exception, e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]

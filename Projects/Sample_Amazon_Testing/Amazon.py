...
def Item_Search(search_text):
    ...
    try:
        ...
        if search_button != "failed":  # if search button found
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Searching successfully done!!!", 1)
            return "passed"
        else:  # if search button is not found
            # Taking Screenshot
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
                sModuleInfo, "Failed to clicked your element", 3)
        ...
        CommonUtil.ExecLog(sModuleInfo, "Could not search item: %s.  Error: %s" % (
            search_text, Error_Detail), 3)
        return "failed"
...
def Add_to_Cart(search_text):
    ...
    try:
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Trying to click on the first search result", 1)
        first_element = BuiltInFunctions.Get_Element(
            'tag', 'a', 'class', 's-item-container', 'parent')
        if first_element != 'failed':
            CommonUtil.ExecLog(
                sModuleInfo, "Found first search result element", 1)
            CommonUtil.ExecLog(
            ...
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
                sModuleInfo, "Clicked on the first search result element successfully", 1)
        else:
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Trying to click on Add to Cart button", 1)
        add_to_cart_button = BuiltInFunctions.Get_Element(
            'id', 'add-to-cart-button')
        if add_to_cart_button != 'failed':
            CommonUtil.ExecLog(sModuleInfo, "Found Add to Cart button", 1)
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
                sModuleInfo, "Clicking on the Add to Cart button", 1)
            add_to_cart_button.click()
            time.sleep(10)
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
                sModuleInfo, "Clicked on the Add to Cart button successfully", 1)
            return "passed"
        else:
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
        ...
        Error_Detail = ((str(exc_type).replace("type ", "Error Type: ")) + ";" + "Error Message: " + str(
            exc_obj) + ";" + "File Name: " + fname + ";" + "Line: " + str(exc_tb.tb_lineno))
        CommonUtil.ExecLog(sModuleInfo, "Could not add the item to cart.  Error: %s" % (
            search_text, Error_Detail), 3)
        return "failed"
...
def Add_to_Cart_Using_Selenium(step_data, file_attachment):
    ...
    try:
        ...
        # Finding the search button
        #search_button = BuiltInFunctions.Get_Element('value','Go')
        search_button = selenium_driver.find_element_by_xpath(
        ...
        if first_element != 'failed':
            ...
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
                sModuleInfo, "Clicked on the first search result elemnt successfully", 1)
        else:
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
                sModuleInfo, "Failed to find the first search result element", 3)
        ...
        add_to_cart_button = selenium_driver.find_element_by_id(
            "add-to-cart-button")
        if add_to_cart_button != 'failed':
            CommonUtil.ExecLog(sModuleInfo, "Found Add to Cart button", 1)
            CommonUtil.TakeScreenShot(sModuleInfo)
        ...
        Error_Detail = ((str(exc_type).replace("type ", "Error Type: ")) + ";" + "Error Message: " + str(
            exc_obj) + ";" + "File Name: " + fname + ";" + "Line: " + str(exc_tb.tb_lineno))
        CommonUtil.ExecLog(sModuleInfo, "Could not add the item %s to cart.  Error: %s" % (
            search_text, Error_Detail), 3)
        return "failed"

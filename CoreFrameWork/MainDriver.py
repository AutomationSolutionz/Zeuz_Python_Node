...
#*****************Start of Main Driver*****************
    ...
    for TestRunID in TestRunLists:
        ...
        for TestCaseID in TestCaseLists:
            ...
            home = os.path.expanduser("~")+'/Desktop/Attachments'
            update_global_config('sectionOne', 'download_folder',home)
            config=ConfigParser.ConfigParser()
            config.read(global_config_file)
            ...
            download_folder=config.get('sectionOne','download_folder')
            ...
            FL.DeleteFolder(config.get('sectionOne','download_folder'))
            FL.CreateFolder(download_folder)
            for each in a_list:
                m=each[1]+'.'+each[2] #file name
                f=open(download_folder+'/'+m,'wb')
                f.write(urllib2.urlopen('http://'+server_id+':'+str(Global.server_port)+each[0]).read())
            ...
            #upload will go here.
            upload_zip(Global.log_file_upload_server_ip,Global.log_file_upload_server_port,config.get('sectionOne','temp_run_file_path'),TestRunID[0],config.get('sectionOne','test_case')+".zip",Global.test_log_base_path)
            TCLogFile=os.sep+Global.test_log_base_path+os.sep+TestRunID[0].replace(":",'-')+'/'+config.get('sectionOne','test_case')+'.zip'
            FL.DeleteFile(config.get('sectionOne','test_case_folder')+'.zip')
        ...
        FL.DeleteFolder(config.get('sectionOne','temp_run_file_path')+os.sep+TestRunID[0].replace(':','-'))
        #End of Test Set
    ...
    return "pass"
#import dropbox
def upload_zip(server_id,port_id,temp_folder,run_id,file_name,base_path):
    """

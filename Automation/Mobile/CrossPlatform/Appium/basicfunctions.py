...
def launch(package_name,activity_name):
    ...
    try:
        ...
        if 'driver' not in globals():
            ...
            try:
                CommonUtil.ExecLog(sModuleInfo,"Now launching the app...",1,local_run)
                desired_caps = {}
...
def install(app_location, app_package):
    sModuleInfo = inspect.stack()[0][3] + " : " + inspect.getmoduleinfo(__file__).name
    try:
        sModuleInfo = inspect.stack()[0][3] + " : " + inspect.getmoduleinfo(__file__).name
        ...
        if 'driver' in globals():
            ...
            try:
                CommonUtil.ExecLog(sModuleInfo,"Now installing and then launching the app...",1,local_run)
                desired_caps = {}
                desired_caps['platformName'] = 'Android'
                df = adbOptions.get_android_version()
                CommonUtil.ExecLog(sModuleInfo,df,1,local_run)

...
try:
    ...
    print(
        Fore.RED
        + "Tkinter is not installed. This is required to start the graphical interface. Please enter the root password to install if asked."
    )
    ...
    if sys.platform == "win32":
        try:
            ...
            print(
                Fore.RED
                + "Failed to install. Please run: pip download pillow & pip install pillow"
            )
        ...
        print(
            s.Popen(
                "sudo apt-get update".split(" "), stdout=s.PIPE, stderr=s.STDOUT
            ).communicate()[0]
        )
        print(
            s.Popen(
                "sudo apt-get -y install python-tk".split(" "),
                stdout=s.PIPE,
                stderr=s.STDOUT,
            ).communicate()[0]
        )
...
class Application(tk.Frame):
    ...
    update_interval = (
        1  # Time in hours to check for updates. We never check less than this time
    )
    ...
    def __init__(self, master=None):
        try:
            ...
            try:
                ...
                pass  # Exception Advanced Options section doesn't exist (old settings format)
    ...
    def createButtons(self):
        try:
            ...
            self.quitButton = tk.Button(
                self.topframe, text="Quit", width=self.button_width, command=teardown
            )
            self.quitButton.grid(row=1, column=0)

            self.settings_selection = tk.StringVar(self)
            self.settings_selection.set("Authentication")
            self.advanced_settings_frames.append(
                "Authentication"
            )  # Need a default value, so we can create the menu
            self.settings_menu = tk.OptionMenu(
                self.topframe, self.settings_selection, *self.advanced_settings_frames
            )  # Create drop down
            self.settings_menu.grid(row=2, column=0)
            self.settings_selection.trace(
                "w", self.show_settings
            ...
            # Node ID
            tk.Label(self.topframe, text="Node ID", fg="blue").grid(
                row=3, column=0, pady=self.pady, sticky="w"
            )
            self.node_id = tk.Entry(
                self.topframe,
                validate="key",
                validatecommand=(self.register(self.onValidate), "%d", "%S"),
            )  # See onValidate() for more info
        ...
        try:
            ...
            if sections:
                for section in sections:  # For each section
                    ...
                    self.settings_menu["menu"].add_command(
                        label=section,
                        command=tk._setit(self.settings_selection, section),
                    )  # Add the team to the drop down menu
                    tk.Label(
                        self.widgets[section]["frame"], text=section, fg="blue"
                    ).grid(
                        row=row, column=0, pady=10, sticky="w", columnspan=2
                    )  # Create Section label
                    row += 1
                    options = ConfigModule.get_all_option(
                        section
                    ...
                    if options:
                        for option in options:  # For each option
                            ...
                            if option == "password":  # Add asterisk to hide password
                                ...
                                self.widgets["Authentication"]["widget"][option][
                                    "widget"
                                ] = tk.Entry(
                                    self.widgets[section]["frame"],
                                    show="*",
                                    width=self.entry_width,
                                )  # Password textbox which hides the password with asterisks
                                self.widgets["Authentication"]["widget"][option][
                                    "widget"
                                ].insert(
                                    "end", value
                            ...
                            # True/False checkbox
                                ...
                                self.widgets[section]["widget"][option][
                                    "check"
                                ] = tk.IntVar()  # Initilize checkbox variable
                                self.widgets[section]["widget"][option][
                                    "widget"
                                ] = tk.Checkbutton(
                                    self.widgets[section]["frame"],
                                    variable=self.widgets[section]["widget"][option][
                                        "check"
                                    ],
                                )  # Checkbox widget
                                if value.lower() == "true":
                                    self.widgets[section]["widget"][option][
                                        ...
                                        1
                                    )  # Enable checkbox if "True"
                                else:
                                    self.widgets[section]["widget"][option][
                                        "check"
                                    ].set(
                                        0
                                    )  # Disable checkbox if "False"
                    ...
                    if section == "Authentication":
                        ...
                        self.startButton = tk.Button(
                            self.widgets[section]["frame"],
                            text="Connect",
                            fg=self.colour_passed,
                            width=self.button_width,
                            command=self.read_mod,
                        )
                        self.startButton.grid(
                            row=row + 1, column=0, columnspan=2, padx=50, pady=10
                        )
        ...
        try:
            ...
            if check:
                ...
                if (
                    ...
                    self.after(
                        self.update_interval * 3600 * 1000,
            ...
            # root.after() brings us here
                ...
                # Some error occurred during updating
                    ...
                    tkinter.messagebox.showerror(
                        "Update",
                        "An error occurred during update: %s"
                        % self_updater.check_complete,
                    )
        except Exception as e:
    ...
    def continuous_server_check(self, check=True):
        ...
        try:
            ...
            if result == False:  # Server likely not configured, or not fully entered
                ...
                user = self.widgets["Authentication"]["widget"]["username"][
                    "widget"
                ].get()
                pw = self.widgets["Authentication"]["widget"]["password"][
                    "widget"
                ].get()
                if user != "" and pw != "":  # User/pass set, so try to login
                    ...
                    if result == False:  # Can't login, try again
                        ...
                        self.widgets["Authentication"]["widget"]["team"][
                            "dropdown"
                        ].set("")
                        self.widgets["Authentication"]["widget"]["project"][
                            "dropdown"
                        ].set("")
    ...
    def password(self, encrypt, key, pw):
        ...
        try:
            ...
            tkinter.messagebox.showerror(
                "Error", "Error decrypting password. Enter a new password {}".format(e)
            )
            return ""
            ...
            for section in self.widgets:
                for option in self.widgets[section]["widget"]:
                    if (
                        ...
                        value = str(
                            self.widgets[section]["widget"][option]["widget"].get()
                        ).strip()  # Get value
                        if option == "password":
                            value = self.password(
                                True, "zeuz", value
    ...
    def get_teams(self, noerror=False):
        ...
        try:
            ...
            for team in self.widgets["Authentication"]["widget"]["team"][
                "choices"
            ]:  # For each new team
                self.widgets["Authentication"]["widget"]["team"]["widget"][
                    "menu"
                ].add_command(
                    label=team,
                    command=tk._setit(
                        self.widgets["Authentication"]["widget"]["team"]["dropdown"],
                        team,
                    ),
                )  # Add the team to the drop down menu
            if (
                ...
                if noerror == False:
                    tkinter.messagebox.showerror(
                        "Error",
                        "No teams could be found. Server, port, username, or password may be wrong",
                    )  # Display an error
                return False
            return True
        except Exception as e:
    ...
    def get_projects(self, team):
        ...
        try:
            self.widgets["Authentication"]["widget"]["project"][
                ...
                team
            )  # Get list of projects for the selected team
            self.widgets["Authentication"]["widget"]["project"]["widget"][
                "menu"
            ].delete(
                0, "end"
            )  # Clear drop down menu
            for project in self.widgets["Authentication"]["widget"]["project"][
                "choices"
                ...
                self.widgets["Authentication"]["widget"]["project"]["widget"][
                    ...
                    label=project,
                    command=tk._setit(
                        self.widgets["Authentication"]["widget"]["project"]["dropdown"],
                        project,
                    ),
                )  # Add the project to the drop down menu
        except Exception as e:

...
def main(server_id):
    ...
    UserList = DBUtil.GetData(
        conn, "Select User_Names from permitted_user_list")
    conn.close()
    if Userid not in UserList:
        CommonUtil.ExecLog(
            sModuleInfo,
            "User don't have permission to run the tests",
            3)
    ...
    conn = DBUtil.ConnectToDataBase(sHost=server_id)
    TestRunLists = DBUtil.GetData(
        conn,
        "Select run_id,rundescription,tester_id from test_run_env Where tester_id = '%s' and (status = 'Submitted')" %
        Userid,
        False)
    conn.close()
    if len(TestRunLists) > 0:
        ...
        CommonUtil.ExecLog(
            sModuleInfo,
            "No Test Run Schedule found for the current user : %s" %
            Userid,
            2)
        return False
    ...
    for TestRunID in TestRunLists:
        ...
        query = "select distinct project_id, team_id from test_run_env tre, machine_project_map mpm where mpm.machine_serial=tre.id and run_id='%s'" % TestRunID[
            0]
        conn = DBUtil.ConnectToDataBase(sHost=server_id)
        project_team = DBUtil.GetData(conn, query, False)
        conn.close()
        dependency_list = collectAlldependency(
            project_team[0][0], project_team[0][1], server_id)
        # code for the dependency_list will go here.
        query = "select rundescription from test_run_env where run_id='%s'" % TestRunID[
            0]
        conn = DBUtil.ConnectToDataBase(sHost=server_id)
        ...
        query = "select distinct name,field,value from machine_run_params mrp, test_run_env tre where tre.id=mrp.machine_serial and run_id='%s'" % (TestRunID[
                                                                                                                                                    0])
        conn = DBUtil.ConnectToDataBase(sHost=server_id)
        ...
        # conn.commit()
        conn = DBUtil.ConnectToDataBase(sHost=server_id)
        DBUtil.UpdateRecordInTable(
            conn, 'test_env_results', "Where run_id = '%s' and tester_id = '%s'" %
            (sTestResultsRunId, Userid), status='In-Progress', teststarttime='%s' %
            (sTestSetStartTime))
        conn.close()
        ...
        # Find all test cases added in the test_run table for the current run
        # id
        conn = DBUtil.ConnectToDataBase(sHost=server_id)
        query = "select distinct tc.tc_id,test_case_type,test_order from result_test_cases tc, test_run tr where tr.run_id=tc.run_id and tr.tc_id=tc.tc_id and tc.run_id='%s' order by tr.test_order" % TestRunID[
            0].strip()
        # "Select TC_ID From test_run Where run_id = '%s'" % TestRunID[0], False)
        TestCaseLists = DBUtil.GetData(conn, query, False)
        conn.close()
        ...
        for each in TestCaseLists:
            print each
            test_case_id = each[0]
            query = "select tc_type from result_test_cases where tc_id='%s' and run_id='%s'" % (
                test_case_id, TestRunID[0])
            conn = DBUtil.ConnectToDataBase(sHost=server_id)
        ...
        if len(filter(lambda x: x[1] == 'Manual', AutomationList)) > 0:
            if (len(filter(lambda x: x[1] == 'Automated', AutomationList))) > 0 or (
                    len(filter(lambda x: x[1] == 'Performance', AutomationList))) > 0:
                run_type = 'Hybrid'
            else:
                run_type = 'Manual'
        else:
            if (len(filter(lambda x: x[1] == 'Performance', AutomationList))) == 0 and (
                    len(filter(lambda x: x[1] == 'Automated', AutomationList))) > 0:
                run_type = 'Automation'
        ...
        automation_count = len(
            filter(
                lambda x: x[1] == 'Automated',
                AutomationList))
        performance_count = len(
            filter(
                lambda x: x[1] == 'Performance',
                AutomationList))
        TestCaseLists = filter(lambda x: x[1] == 'Automated' or x[
                               1] == 'Performance', AutomationList)
        print TestCaseLists
        if len(TestCaseLists) > 0:
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                "No test cases found for the current user : %s" %
                Userid,
                2)
            return False
        ...
        for TestCaseID in TestCaseLists:
            print TestCaseID
            # first check that it is all copied otherwise check it
            # periodically.
            test_case_copy_status = False
            while(test_case_copy_status != True):
                query = "select copy_status from test_run where tc_id='%s' and run_id='%s'" % (
                    TestCaseID[0], TestRunID[0])
                Conn = DBUtil.ConnectToDataBase(sHost=server_id)
            ...
            """
            conn = DBUtil.ConnectToDataBase(sHost=server_id)
            TestCaseName = DBUtil.GetData(
                conn, "Select tc_name From result_test_cases Where tc_id = '%s' and run_id='%s'" %
                (TCID, TestRunID[0]), False)
            conn.close()
            ...
            test_case_folder = log_file_path + os.sep + \
                (TestRunID[0].replace(':', '-') + os.sep + TCID.replace(":", '-'))
            ConfigModule.add_config_value(
                'sectionOne', 'test_case', TCID, temp_ini_file)
            ConfigModule.add_config_value(
                'sectionOne',
                'test_case_folder',
                test_case_folder,
            ...
            home = os.path.join(
                FL.get_home_folder(), os.path.join(
                    'Desktop', 'Attachments'))
            ConfigModule.add_config_value(
            ...
            query = "select distinct file_path,file_name,file_type,sa.step_id from step_attachment sa, result_test_steps rts where rts.step_id=sa.step_id and rts.run_id='%s' and rts.tc_id='%s'" % (TestRunID[
                                                                                                                                                                                                     0], TCID)
            conn = DBUtil.ConnectToDataBase(sHost=server_id)
            ...
            for each in a_list:
                ...
                f.write(urllib2.urlopen('http://' +
                                        ConfigModule.get_config_value('Server', 'server_address') +
                                        ':' +
                                        str(ConfigModule.get_config_value('Server', 'server_port')) +
                                        '/static' +
                                        each[0]).read())
            ...
            for each in ta_list:
                ...
                file_specific_steps.update(
                    {m: download_folder + '/' + str(each[3]) + '/' + m})
                f.close()
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "-------------*************--------------", 1)
            CommonUtil.ExecLog(
                sModuleInfo, "Running Test case id : %s :: %s" %
                (TCID, TestCaseName), 1)
            ...
            conn = DBUtil.ConnectToDataBase(sHost=server_id)
            TestCaseResultIndex = DBUtil.GetData(
                conn, "Select id From test_case_results where run_id = '%s' and TC_ID = '%s' Order By id desc limit 1" %
                (sTestResultsRunId, TCID), False)
            conn.close()
            ...
            """
            conn = DBUtil.ConnectToDataBase(sHost=server_id)
            TestStepsList = DBUtil.GetData(
                conn,
                "Select ts.step_id,stepname,teststepsequence,tsl.driver,tsl.steptype,tsl.data_required,tsl.step_editable From result_test_steps ts,result_test_steps_list tsl where ts.run_id=tsl.run_id and TC_ID = '%s' and ts.step_id = tsl.step_id and tsl.stepenable='true' and ts.run_id='%s' Order By teststepsequence" %
                (TCID,
                 TestRunID[0]),
                False)
            conn.close()
            ...
            """
            conn = DBUtil.ConnectToDataBase(sHost=server_id)
            DataSetList = DBUtil.GetData(
                conn, "Select tcdatasetid from result_test_case_datasets where tc_id='%s' and run_id='%s'" %
                (TCID, TestRunID[0]), False)
            conn.close()
            if len(DataSetList) == 0:
                # This condition is for test cases which dont have any input
                # data
            ...
            for EachDataSet in DataSetList:
                ...
                while StepSeq <= Stepscount:
                    ...
                    Conn = DBUtil.ConnectToDataBase(sHost=server_id)
                    query = "select description from master_data where field='continue' and value='point' and id ='%s'" % (
                        TCID + '_s' + str(StepSeq))
                    ...
                    ConfigModule.add_config_value(
                        'sectionOne',
                        'sTestStepExecLogId',
                        sTestResultsRunId + "|" + TCID + "|" + str(
                            TestStepsList[
                                StepSeq - 1][0]) + "|" + str(StepSeq),
                        temp_ini_file)
                    # Test Step start time
                    conn = DBUtil.ConnectToDataBase(sHost=server_id)
                    now = DBUtil.GetData(
                        conn, "SELECT CURRENT_TIMESTAMP;", False)
                    conn.close()
                    ...
                    # conn.commit()
                    condition = "where run_id='" + sTestResultsRunId + "' and tc_id='" + TCID + "' and teststep_id='" + \
                        str(TestStepsList[StepSeq - 1][0]) + "' and teststepsequence='" + str(TestStepsList[StepSeq - 1][2]) + "'"
                    Dict = {
                        'teststepsequence': TestStepsList[
                            StepSeq - 1][2],
                        'status': 'In-Progress',
                        ...
                        'testcaseresulttindex': TestCaseResultIndex[0][0]}
                    conn = DBUtil.ConnectToDataBase(sHost=server_id)
                    DBUtil.UpdateRecordInTable(
                        conn, "test_step_results", condition, **Dict)
                    conn.close()
                    steps_data = []
                    ...
                    """if TestStepsList[StepSeq-1][5] and TestStepsList[StepSeq-1][6]:
                        ...
                        steps_data=[]"""
                    container_id_data_query = "select ctd.curname,ctd.newname from test_steps_data tsd, container_type_data ctd where tsd.testdatasetid = ctd.dataid and tcdatasetid = '%s' and teststepseq = %d and ctd.curname Ilike '%%_s%s%%'" % (EachDataSet[
                                                                                                                                                                                                                                                      0], int(TestStepsList[StepSeq - 1][2]), StepSeq)
                    conn = DBUtil.ConnectToDataBase(sHost=server_id)
                    container_data_details = DBUtil.GetData(
                        conn, container_id_data_query, False)
                    conn.close()
                    steps_data = []
                    for each_data_id in container_data_details:
                        From_Data = DataFetching.Get_Data_Like_Server(
                            server_id, TestRunID[0].strip(), each_data_id[0])
                        steps_data.append(From_Data)
                    print "steps data for #%d: " % StepSeq, steps_data
                    CommonUtil.ExecLog(
                        sModuleInfo, "steps data for #%d: %s" %
                        (StepSeq, str(steps_data)), 1)
                    # get the estimateed time for the steps
                    step_time_query = "select description from result_master_data where field='estimated' and value='time' and run_id='%s' and id='%s'" % (
                        TestRunID[0].strip(), (TCID + '_s' + str(StepSeq)).strip())
                    conn = DBUtil.ConnectToDataBase(sHost=server_id)
                    ...
                    try:
                        ...
                        if TestStepsList[StepSeq - 1][3] in Driver_list:
                            module_name = importlib.import_module(
                                TestStepsList[StepSeq - 1][3])
                            step_name = TestStepsList[StepSeq - 1][1]
                            step_name = step_name.lower().replace(' ', '_')
                            if test_case_type == 'Performance':
                                config_file_path = os.path.realpath(os.path.join(
                                    (os.path.join(module_name.__file__, os.pardir)), os.pardir))
                                conf = config_file_path + os.sep + 'ConfigFiles' + \
                                    os.sep + TestStepsList[StepSeq - 1][3] + '.conf'
                                functionTocall = getattr(
                                ...
                                dep_plus_run_params = {}
                                dep_plus_run_params.update(
                                    {'dependency': dependency_list_final})
                                dep_plus_run_params.update(
                                    {'run_time_params': run_para})
                                functionTocall = getattr(
                                    module_name, step_name)
                                simple_queue = Queue.Queue()
                                if ConfigModule.get_config_value(
                                        'RunDefinition', 'Threading') in passed_tag_list:
                                    stepThread = threading.Thread(target=functionTocall, args=(
                                        dep_plus_run_params, steps_data, file_specific_steps, simple_queue))
                                else:
                        ...
                        if ConfigModule.get_config_value(
                            ...
                            print "Waiting for Test Step Thread to finish..for (seconds) :", step_time
                            # Global.DefaultTestStepTimeout
                            ...
                            try:
                                ...
                                # Global.DefaultTestStepTimeout
                                print "Test Step did not return after default timeout (secs) : ", step_time
                                sStepResult = "Failed"
                                q.put(sStepResult)
                                # Clean up
                        ...
                        print Error_Detail
                        CommonUtil.ExecLog(
                            sModuleInfo, "Exception occurred in test step : %s" %
                            Error_Detail, 3)
                        sStepResult = "Failed"
                    ...
                    try:
                        ...
                        print Error_Detail
                        CommonUtil.ExecLog(
                            sModuleInfo, "Exception closing DB connection:%s" %
                            Error_Detail, 2)
                    # test Step End time
                    conn = DBUtil.ConnectToDataBase(sHost=server_id)
                    now = DBUtil.GetData(
                    ...
                    # add result of each step to a list; for a test case to
                    # pass all steps should be pass; atleast one Failed makes
                    # it 'Fail' else 'Warning' or 'Blocked'
                    if sStepResult:
                    ...
                    if sStepResult.upper() == "PASSED":
                        ...
                        CommonUtil.ExecLog(
                            sModuleInfo, "%s : Test Step Passed" %
                            TestStepsList[
                                StepSeq - 1][1], 1)
                        ...
                        DBUtil.UpdateRecordInTable(
                            conn,
                            'test_step_results',
                            "Where run_id = '%s' and tc_id = '%s' and teststep_id = '%s' and teststepsequence = '%d' and testcaseresulttindex = '%d'" %
                            (sTestResultsRunId,
                             TCID,
                             TestStepsList[
                                 StepSeq -
                        ...
                        # Step has Warning, but continue running next test step
                        # for this test case
                        print TestStepsList[StepSeq - 1][1] + ": Test Step Warning"
                        CommonUtil.ExecLog(
                            sModuleInfo, "%s : Test Step Warning" %
                            TestStepsList[
                                StepSeq - 1][1], 2)
                        # Update Test Step Results table
                        conn = DBUtil.ConnectToDataBase(sHost=server_id)
                        ...
                        CommonUtil.ExecLog(
                            sModuleInfo, "%s : Test Step Not Run" %
                            TestStepsList[
                                StepSeq - 1][1], 2)
                        # Update Test Step Results table
                        conn = DBUtil.ConnectToDataBase(sHost=server_id)
                        ...
                        # Step has a Critial failure, fail the test step and
                        # test case. go to next test case
                        print TestStepsList[StepSeq - 1][1] + ": Test Step Failed Failure"
                        CommonUtil.ExecLog(
                            sModuleInfo, "%s : Test Step Failed Failure" %
                            TestStepsList[
                                StepSeq - 1][1], 3)
                        # Update Test Step Results table
                        conn = DBUtil.ConnectToDataBase(sHost=server_id)
                        ...
                        # Step is Blocked, Block the test step and test case.
                        # go to next test case
                        print TestStepsList[StepSeq - 1][1] + ": Test Step Blocked"
                        CommonUtil.ExecLog(
                            sModuleInfo, "%s : Test Step Blocked" %
                            TestStepsList[
                                StepSeq - 1][1], 3)
                        # Update Test Step Results table
                        conn = DBUtil.ConnectToDataBase(sHost=server_id)
                        ...
                        CommonUtil.ExecLog(
                            sModuleInfo, "%s : Test Step Cancelled" %
                            TestStepsList[
                                StepSeq - 1][1], 3)
                    ...
                    # Check if Test Set status is 'Cancelled' When it is
                    # stopped from Website
                    conn = DBUtil.ConnectToDataBase(sHost=server_id)
                    currentTestSetStatus = DBUtil.GetData(
                        conn, "Select status"
                        " From test_run_env"
                        " Where run_id = '%s'" %
                        sTestResultsRunId, False)
                    conn.close()
            ...
            if 'BLOCKED' in sTestStepResultList:
                ...
                if status[0][0] == "yes":
                    ...
                    sTestCaseStatus = 'Blocked'
                CommonUtil.ExecLog(
                    sModuleInfo, "Test Case " + sTestCaseStatus, 3)
            elif 'WARNING' in sTestStepResultList:
                print "Test Case Contain Warning(s)"
                CommonUtil.ExecLog(
                    sModuleInfo, "Test Case Contain Warning(s)", 2)
                sTestCaseStatus = "Failed"
            ...
            upload_zip(
                ...
                ConfigModule.get_config_value(
                    'Temp',
                    '_file_upload_path'))
            TCLogFile = os.sep + ConfigModule.get_config_value('Temp',
                                                               '_file_upload_path') + os.sep + TestRunID[0].replace(":",
                                                                                                                    '-') + '/' + ConfigModule.get_config_value('sectionOne',
                                                                                                                                                               'test_case',
                                                                                                                                                               temp_ini_file) + '.zip'
            FL.DeleteFile(
                ConfigModule.get_config_value(
            ...
            try:
                conn = DBUtil.ConnectToDataBase(sHost=server_id)
                FailReason = CommonUtil.FindTestCaseFailedReason(
                    conn, sTestResultsRunId, TCID)
                conn.close()
            except Exception as e:
                exc_type, exc_obj, exc_tb = sys.exc_info()
                fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            ...
            DBUtil.UpdateRecordInTable(
                ...
                (TCLogFile))
            conn.close()
            # Update Performance Results if Its a Performance test case And if
            # test case had Passed
            if DataType == 'Performance' and sTestCaseStatus == 'Passed':
                ...
                conn = DBUtil.ConnectToDataBase(sHost=server_id)
                DBUtil.InsertNewRecordInToTable(
                    conn,
                    'performance_results',
                    ...
                    hw_model=hwmodel)
                conn.close()
            # Check if Test Set status is 'Cancelled' When it is stopped from
            # Website
            conn = DBUtil.ConnectToDataBase(sHost=server_id)
            currentTestSetStatus = DBUtil.GetData(
                conn, "Select status"
            ...
            if currentTestSetStatus == 'Cancelled':
                ...
                CommonUtil.ExecLog(
                    sModuleInfo,
                    "Test Run status is Cancelled. Exiting the current Test Set...%s" %
                    sTestResultsRunId,
                    2)
                break
        ...
        TestSetDuration = CommonUtil.FormatSeconds(
            (iTestSetEndTime - iTestSetStartTime).seconds)
        ...
        if currentTestSetStatus == 'Cancelled':
            ...
            DBUtil.UpdateRecordInTable(
                conn,
                'test_env_results',
                "Where run_id = '%s' and tester_id = '%s'" %
                (sTestResultsRunId,
                 Userid),
                status='Cancelled',
                testendtime='%s' %
            ...
            if (automation_count > 0 or performance_count > 0) and (
                    automation_count + performance_count) == len(TestCaseLists):
                conn = DBUtil.ConnectToDataBase(sHost=server_id)
                ...
                conn.close()
                # Update test_run_env schedule table with status so that this
                # Test Set will not be run again
                conn = DBUtil.ConnectToDataBase(sHost=server_id)

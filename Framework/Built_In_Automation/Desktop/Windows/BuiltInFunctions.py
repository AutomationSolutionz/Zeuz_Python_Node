...
import System
import inspect
import win32con  # What is this for?
import win32api
import os
import clr
import autoit
...
def Click_Element(data_set):
    ...
    try:
        result = Click_Element_None_Mouse(
            Element, expand, invoke, select, toggle)
        CommonUtil.TakeScreenShot(sModuleInfo)
        if result in failed_tag_list:
            CommonUtil.ExecLog(sModuleInfo, "Could not click element", 3)
            return 'failed'
...
def Right_Click_Element(data_set):
    ...
    try:
        ...
        if Element in failed_tag_list:
            CommonUtil.ExecLog(sModuleInfo, "Could not find element", 3)
            return 'failed'
        x = (int)(Element.Current.BoundingRectangle.Right -
                  Element.Current.BoundingRectangle.Width / 2)
        y = (int)(Element.Current.BoundingRectangle.Bottom -
    ...
    try:
        ...
        try:
            if isinstance(MainWindowName_OR_ParentElement, basestring) == True:
                ParentElement = _get_main_window(
                ...
                if ParentElement == None:
                    ...
                    ChildElement = _child_search(
                        ParentElement, Element_Name, Element_Class, Element_AutomationID, Element_LocalizedControlType)
        ...
        print ((str(exc_type).replace("type ", "Error Type: ")) + ";" + "Error Message: " +
    ...
    try:
        ...
        try:
            ...
            for each_child in child_elements:
                ...
                child = _child_search(each_child, Element_Name, Element_Class,
                                      Element_AutomationID, Element_LocalizedControlType)
                if child:
                    return child
...
def Validate_Text(data_set):
    ...
    try:
        ...
        actual_text = str(Element.GetCurrentPattern(
            ValuePattern.Pattern).Current.Value).strip().lower()
...
def Save_Text(data_set):
    ...
    try:
        ...
        if field == 'value':
            actual_text = str(Element.GetCurrentPattern(
                ValuePattern.Pattern).Current.Value).strip()
        elif field == 'name':
            actual_text = str(Element.GetCurrentPattern(
                ValuePattern.Pattern).Current.Name).strip()
        elif field == 'class':
            actual_text = str(Element.GetCurrentPattern(
                ValuePattern.Pattern).Current.ClassName).strip()
        elif 'id' in field:
            actual_text = str(Element.GetCurrentPattern(
                ValuePattern.Pattern).Current.AutomationId).strip()
        elif 'type' in field or 'control' in field:
            actual_text = str(Element.GetCurrentPattern(
                ValuePattern.Pattern).Current.LocalizedControlType).strip()

        Shared_Resources.Set_Shared_Variables(variable_name, actual_text)

        CommonUtil.ExecLog(sModuleInfo, "Text '%s' is saved in the variable '%s'" % (
            actual_text, variable_name), 1)
        return "passed"
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info(), None, "Error parsing data set")
...
def Get_Element(data_set):
    ...
    try:
        ...
        # Get element object
        Element = get_element(window_name, element_name,
                              element_class, automationid, control_type, wait_time)
        if Element == None:
            return "failed"
...
def Run_Application(data_set):
    ...
    try:
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Unable to start your app %s" % Desktop_app, 3)
        return "failed"

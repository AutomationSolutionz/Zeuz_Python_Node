...
from Framework.Built_In_Automation.Sequential_Actions.common_functions import shared_variable_to_value
...
@logger
def Windows_Conditional_Action_Handler(step_data, dataset_cnt):
    ...
    try:
        ...
        for left, mid, right in data_set:
            ...
            if "recall" in left and "result" in mid:  # If Field = recall and Sub-Field = result
                ...
                actions_for_true = sequential_actions.get_data_set_nums(right)
        sequential_actions.load_sa_modules(module)
    ...
    for left, mid, right in data_set:
        ...
        if "true" in left and "conditional action" in mid:
            outer_skip += sequential_actions.get_data_set_nums(str(right).strip())
            if logic_decision == "true":
                for i in sequential_actions.get_data_set_nums(str(right).strip()):
                    next_level_step_data.append(i)
                log_msg = sequential_actions.if_else_log_for_actions(log_msg, next_level_step_data, "element")
    ...
    for data_set_index in next_level_step_data:
        ...
        if data_set_index not in inner_skip:
            result, skip = sequential_actions.Run_Sequential_Actions(
                [data_set_index]
            )  # Running

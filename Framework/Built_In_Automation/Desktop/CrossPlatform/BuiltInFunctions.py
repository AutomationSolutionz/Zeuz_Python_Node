...
def get_exec_from_icon(file_name):
    ...
    try:
        ...
        for element in data:
            if element[:5] == "Exec=":
                # Save execution line without the Exec= part
                result = element[5:].strip()
...
def Enter_Text(data_set):
    ...
    try:
        ...
        if text_value == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Could not find value for this action", 3)
            return 'failed'
    except:
    ...
    try:
        ...
        if element_parameter:
            ...
            if x in failed_tag_list:  # Error reason logged by Get_Element
                CommonUtil.ExecLog(
                    sModuleInfo, "Error calculating coordinates", 3)
                return 'failed'
            CommonUtil.ExecLog(
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "No element provided. Assuming textbox has keyboard focus", 0)
...
def close_program(data_set):
    ...
    try:
        ...
        if program_name == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Expected a program name in Value", 3)
            return 'failed'
    except Exception:
        errMsg = "Error parsing data set"
        return CommonUtil.Exception_Handler(sys.exc_info(), None, errMsg)
    ...
    try:
        if dependency['PC'].lower() == 'linux' or dependency['PC'].lower() == 'mac':
            ...
            command = 'pkill -f ' + program_name
            # Execute command and return the return code
            close_status, output = FU.run_cmd(command, return_status=True)
            ...
            # pkill failed, try another method
                ...
                # Try to find the PID
                command = "ps aux | grep -i '%s' | grep -v grep | awk '{print $2}'" % program_name
                ...
                if output != []:
                    ...
                    close_status = FU.run_cmd("kill -9 %s" % output)
                else:
...
def move_mouse(data_set):
    ...
    try:
        ...
        if cmd == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Valid action not found. Expected Field set to 'move' or 'hover'", 3)
            return 'failed'

        if cmd == 'hover':
            if file_name == '':
                CommonUtil.ExecLog(
                    sModuleInfo, "Valid element not found. Expected Sub-Field to be 'element parameter', and Value to be a filename", 3)
                return 'failed'
            if position not in positions:
                CommonUtil.ExecLog(
                    sModuleInfo, "Will click on centre of element. Expected Value to be one of: %s" % str(positions), 2)
                position = 'centre'
        elif cmd == 'move':
            if file_name == '':
                CommonUtil.ExecLog(
                    sModuleInfo, "Valid action not found. Expected Value to be coordinates in format of 'x,y'", 3)
                return 'failed'
    ...
    try:
        if cmd == 'hover':
            ...
            try:
                if ',' not in file_name:
                    CommonUtil.ExecLog(
                        sModuleInfo, "Expected Value to be 'X,Y' format for coordinates. If you want to use an image, try using the 'hover' action", 3)
                    return 'failed'

                x, y = file_name.replace(' ', '').split(
                    ',')  # Get the coordinates
                x = int(x)
                y = int(y)
            except:
...
def Click_Element(data_set):
    ...
    try:
        ...
        if cmd == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Valid action not found. Expected Field set to 'click' or 'doubleclick', and the Value one of: %s" % str(positions), 3)
            return 'failed'
        if position not in positions:
            CommonUtil.ExecLog(
                sModuleInfo, "Will click on centre of element. Expected Value to be one of: %s" % str(positions), 2)
...
def launch_program(data_set):
    ...
    try:
        ...
        if file_name in file_attachment:
            ...
            Command = file_attachment[file_name]
        ...
        if dependency['PC'].lower() == 'linux' or dependency['PC'].lower() == 'mac':
            ...
            launch_status = subprocess.Popen(Command.split(' '))
        ...
        if launch_status in failed_tag_list:
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Program launched successfully.", 1)
            return 'passed'
...
def Drag_Element(data_set):
    ...
    try:
        ...
        if src_file_name == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Valid element not found. Expected Sub-Field to be 'source parameter', and Value to be a filename", 3)
            return 'failed'
    ...
    try:
        ...
        result = gui.dragTo(dst_x, dst_y, 2, button='left')
...
def navigate_listbox(data_set):
    ...
    try:
        ...
        for i in range(max_tries):
            ...
            if element in failed_tag_list:  # Error reason logged by Get_Element
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not locate element - trying a new position. Attempt #%d" % i, 0)
                gui.hotkey('pgdn')

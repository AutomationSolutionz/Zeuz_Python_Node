...
def remove_a_desktop_folder(step_data):
    ...
    try:
        if ((len(step_data[0]) >= 5)):
            CommonUtil.ExecLog(sModuleInfo,"The information in the data-set(s) are incorrect. Please provide accurate data set(s) information.",
                                   3)
            return "failed"
        else:
            file_location = FL.get_home_folder() + step_data[0][0][2]
            result = CommonUtil.run_cmd('rm -rf ' + file_location)
            CommonUtil.ExecLog(sModuleInfo, "Successfully deleted folder %s", 1)
            return result
        ...
        error_detail = ((str(exc_type).replace("type ", "Error Type: ")) + ";" + "Error Message: " + str(exc_obj) +
                    ";" + "File Name: " + fname + ";" + "Line: " + str(exc_tb.tb_lineno))
        CommonUtil.ExecLog(sModuleInfo, "Unable to delete folder. Error: %s" % error_detail, 3, local_run)
        return Failed
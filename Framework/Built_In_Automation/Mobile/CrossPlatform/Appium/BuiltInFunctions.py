...
def PATH_(p): return os.path.abspath(
...
if Shared_Resources.Test_Shared_Variables('file_attachment'):
    file_attachment = Shared_Resources.Get_Shared_Variables(
        'file_attachment')  # Retreive file attachments

# Recall appium details
# Check if driver is already set in shared variables
if Shared_Resources.Test_Shared_Variables('appium_details'):
    appium_details = Shared_Resources.Get_Shared_Variables(
        'appium_details')  # Retreive appium driver
...
if Shared_Resources.Test_Shared_Variables('device_info'):
    device_info = Shared_Resources.Get_Shared_Variables(
        'device_info')  # Retreive device_info
...
def find_appium():
    ...
    appium_list = [
        ...
        os.path.join(str(os.getenv('USERPROFILE')), 'AppData',
                     'Roaming', 'npm', 'appium.cmd')
    ...
    if appium_binary == '':  # Didn't find where appium was installed
        ...
        for exe in ('appium', 'appium.exe', 'appium.bat', 'appium.cmd'):
            # Get path and search for executable with in
            result = find_exe_in_path(exe)
            if result != 'failed':
    ...
    if appium_binary == '':  # Didn't find where appium was installed
        ...
        if appium_binary == '':  # Didn't find where appium was installed
            ...
            CommonUtil.ExecLog(sModuleInfo, "Found appium: %s" %
                               appium_binary, 1)
...
def find_correct_device_on_first_run(serial_or_name, device_info):
    ...
    try:
        ...
        if len(all_device_info) == 0:
            CommonUtil.ExecLog(
                sModuleInfo, "Could not detect any connected devices. Ensure at least one is attached via USB, and that it is authorized - Trusted / USB Debugging enabled", 3)
            return 'failed'
        ...
        for device in all_device_info:  # For each device serial number
            if serial_or_name.lower() == device.lower():
                ...
                device_type = all_device_info[device]['type'].lower()
        ...
        if serial_check == False:
            ...
            # Lastly, if nothing above is set, the user did not specify anything, and we have no information from the server. Pick a connected device, and fail if there are none
                ...
                for device in devices:
                    ...
                    CommonUtil.ExecLog(
                        sModuleInfo, "No device information found. Picked one that is connected: %s" % serial, 0)
        ...
        if serial != '' and device_type != '' and did != '':
            ...
            if did in appium_details:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "If test case fails, please quit node, and make sure you have added tear down in your test.", 2)
                return 'passed'

            # Verify this device is actually connected
            if not serial_in_devices(serial, all_device_info):
                CommonUtil.ExecLog(
                    sModuleInfo, "Although we have a selected device, it did not appear in the list of connected devices. Please ensure the device information aligns with what is connected: %s (%s)" % (did, serial), 3)
                return 'failed'
            ...
            if 'driver' not in appium_details[device_id]:
                # Initialize appium driver object
            ...
            CommonUtil.ExecLog(sModuleInfo, "Matched provided device identifier as %s (%s)" % (
                device_id, serial), 1)
            return 'passed'
        else:
            CommonUtil.ExecLog(sModuleInfo, "Although we found connected devices, provided information could not get all required information. Found devices: %s | Deployed device list: %s" % (
                str(devices), str(device_info)), 3)
            return 'failed'
    ...
    try:
        ...
        if appium_details[device_id]['type'] == 'android':
            ...
            if result in failed_tag_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Couldn't unlock the android device", 3)
                return 'failed'
            else:
                CommonUtil.ExecLog(
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "The device type is not android", 3)
            return 'failed'
    ...
    try:
        ...
        if appium_details[device_id]['type'] == 'android':
            ...
            if result in failed_tag_list:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "Unlocked your app successfully", 1)
                return "passed"
        else:
            CommonUtil.ExecLog(
                sModuleInfo, "The device type is not android", 3)
...
def launch_application(data_set):
    ...
    try:
        ...
        if activity_name == '':
            if appium_details[device_id]['type'] == 'android':
                ...
                package_name, activity_name = get_program_names(package_name)
                Shared_Resources.Set_Shared_Variables(
    ...
    try:
        ...
        if appium_details[device_id]['driver'] == None:
            result, launch_app = start_appium_driver(package_name, activity_name, platform_version=platform_version,
                                                     device_name=device_name, ios=ios, no_reset=no_reset, work_profile=work_profile)
            if result == 'failed':
                return 'failed'
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Launched the application successfully.", 1)
        return "passed"
    except Exception:
...
def start_appium_server():
    ...
    try:
        ...
        if appium_details[device_id]['driver']:
            # Get the subprocess object
            appium_server = appium_details[device_id]['driver']
            try:
        ...
        # Increment the port number (by 2 because adb seems to grab the next port), for the next time we run, so we can have multiple instances
        appium_port += 2
        wdaLocalPort += 2
        try:
            appium_server = None
            if sys.platform == 'win32':  # We need to open appium in it's own command dos box on Windows
                ...
                cmd = 'start "Appium Server" /wait /min cmd /c %s -p %d' % (
                    appium_binary, appium_port)
                # Needs to run in a shell due to the execution command
                appium_server = subprocess.Popen(cmd, shell=True)
            elif sys.platform == 'darwin':
                appium_server = subprocess.Popen(
                    [appium_binary, '-p', str(appium_port)])
            else:
                try:
                    ...
                    appium_server = subprocess.Popen(
                        ['appium', '-p', str(appium_port)], env=env)
                except:
                    CommonUtil.ExecLog(
                        sModuleInfo, "Couldn't launch appium server, please do it manually ny typing 'appium &' in the terminal", 2)
                    pass
            # Save the server object for teardown
            appium_details[device_id]['server'] = appium_server
        ...
        CommonUtil.ExecLog(sModuleInfo, "Waiting for server to start on port %d: %s" % (
            appium_port, appium_binary), 0)
        ...
        while True:  # Dynamically wait for appium to start by polling it
            ...
            try:  # If this works, then stop waiting for appium
                r = requests.get('http://localhost:%d/wd/hub/sessions' %
                                 appium_port)  # Poll appium server
                if r.status_code:
                    break
            except:
...
def start_appium_driver(package_name='', activity_name='', filename='', platform_version='', device_name='', ios='', no_reset=False, work_profile=False):
    ...
    try:
        ...
        if appium_details[device_id]['driver'] == None:
            ...
            desired_caps = {}
            # Set platform name
            desired_caps['platformName'] = appium_details[device_id]['type']
            desired_caps['autoLaunch'] = 'false'  # Do not launch application
            # Do not clear application cache when complete
            desired_caps['fullReset'] = 'false'
            # Do not clear application cache when complete
            desired_caps['noReset'] = 'true'
            # Command timeout before appium destroys instance
            ...
            if str(appium_details[device_id]['type']).lower() == 'android':
                ...
                if appium_details[device_id]['imei'] == 'Simulated':  # ios simulator
                    ...
                    if Shared_Resources.Test_Shared_Variables('ios_simulator_folder_path'):
                    ...
                    bundle_id = str(subprocess.check_output(
                        ['osascript', '-e', 'id of app "%s"' % str(app)])).strip()
                    desired_caps = {}
                    # Use set_value() for writing to element
                    desired_caps['app'] = app
                    # Read version #!!! Temporarily hard coded
                    ...
                    # Use set_value() for writing to element
                    desired_caps['sendKeyStrategy'] = 'setValue'
                    # Read version #!!! Temporarily hard coded
                    desired_caps['platformVersion'] = '10.3'
                    # Read model (only needs to be unique if using more than one)
                    desired_caps['deviceName'] = 'iPhone'
                    desired_caps['bundleId'] = ios
                    # Device unique identifier - use auto if using only one phone
                    desired_caps['udid'] = appium_details[device_id]['serial']
            else:
                CommonUtil.ExecLog(sModuleInfo, "Invalid device type: %s" % str(
                    appium_details[device_id]['type']), 3)
                return 'failed', launch_app
            CommonUtil.ExecLog(sModuleInfo, "Capabilities: %s" %
                               str(desired_caps), 0)
            ...
            try:
                ...
                while count <= 5:
                    try:
                        appium_driver = webdriver.Remote(
                            'http://localhost:%d/wd/hub' % appium_port, desired_caps)  # Create instance
                        if appium_driver:
                            break
                        count += 1
                        time.sleep(10)
                ...
                if appium_driver:  # Make sure we get the instance
                    ...
                    appium_driver = None
                    CommonUtil.ExecLog(
                        sModuleInfo, "Error during Appium setup", 3)
                    return 'failed', launch_app
            except Exception, e:
                print e
...
def kill_appium_on_windows(appium_server):
    ...
    try:
        ...
        for child in psutil.Process(appium_server.pid).children(recursive=True):
            try:
                cpid = int(str(child.as_dict(attrs=['pid'])[
                           'pid']).replace("'", ""))  # Get child PID
                CommonUtil.ExecLog(
                    sModuleInfo, "Killing Appium child: %d" % cpid, 0)
                psutil.Process(cpid).send_signal(
                    signal.SIGTERM)  # Send kill to it
                #print h.terminate()
            except:
                pass
    except Exception:
    ...
    try:
        for name in appium_details:  # For each connected device
            try:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "Error destroying Appium instance/server for %s - may already be killed" % name, 2)
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Error destroying Appium instance/server - may already be killed", 2)
    ...
    try:
        ...
        for row in data_set:  # Find required data
            if row[1] == 'action':  # If using format of package on action line, and no serial number
                # May be serial or filename, we'll figure out later
                serial = row[2].strip()
        ...
        if file_name == '':  # Fix previous filename from action row if no element parameter specified
            # There was no element parameter row, so take the action row value for the filename
            file_name = serial
            serial = ''
        ...
        if file_name not in file_attachment and os.path.exists(file_name) == False:
            CommonUtil.ExecLog(
                sModuleInfo, "Could not find file attachment called %s, and could not find it locally" % file_name, 3)
            return 'failed'
        ...
        if file_name == '':
            CommonUtil.ExecLog(
                sModuleInfo, "File not specified or there was a problem reading the file attachments", 3)
            return 'failed'
...
def swipe_in_direction(data_set):
    ...
    try:
        ...
        final_search_string = '%s.%s%s.%s%s' % (
            parent_string, child_string, index_string, horizontal_string, direction)
        try:
            appium_driver.find_element_by_android_uiautomator(
                final_search_string)
        except:
            pass
        CommonUtil.ExecLog(
            sModuleInfo, "Swiped to the element successfully", 1)
...
def swipe_handler_android(data_set):
    ...
    def Calc_Swipe(w, h, inset, direction, position, exact):
        ...
        try:
            ...
            if exact != '':  # User specified exact coordinates, so use those and nothing else
                ...
                inset = int(str(inset).replace('%', '')) / \
                    100.0  # Convert % to float
                position = int(str(position).replace('%', '')) / \
                    100.0  # Convert % to float

                if direction == 'left':
                    # Calculate from other end (X% from max width)
                    tmp = 1.0 - inset
                    inset = int(tmp * w)
                    position = int(position * h)
                elif direction == 'down':
                    # Convert into pixels for that direction
                    inset = int(inset * h)
                    ...
                    # Calculate from other end (X% from max height)
                    tmp = 1.0 - inset
                    inset = int(tmp * h)
                    position = int(position * w)
            ...
            return x1, x2, y1, y2, inset, position
        except Exception:
    ...
    try:
        # Use appium swipe instead of adb swipe (which is used when there's a vitual navigation bar that we need to swipe under)
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Screen size (WxH): %d x %d" % (w, h), 0)
    except Exception:
    ...
    try:
        ...
        if inset == '' or direction not in ('left', 'right', 'up', 'down') or position == '':
            if exact == '':  # If this is set, then the others don't matter, so continue with the gesture
                CommonUtil.ExecLog(
                    sModuleInfo, "Missing critical swipe values. Either 'inset' (optional), 'direction' (required), or 'position' (optional) are missing, wrong or blank", 3)
                return 'failed'
        ...
        if Element:
            ...
            CommonUtil.ExecLog(sModuleInfo, "Element X, Y, W, H: %d, %d, %d, %d" % (
                elementX, elementY, elementW, elementH), 0)
    ...
    try:
        if exact != '':
            CommonUtil.ExecLog(sModuleInfo, "Performing an exact swipe using coordinates: %d, %d to %d, %d %d ms" % (
                x1, y1, x2, y2, duration), 1)
        else:
            CommonUtil.ExecLog(sModuleInfo, "Performing calculated swipe gesture based on inset: %d, position: %d, direction: %s, calculated as %d, %d to %d, %d %d ms" % (
                inset, position, direction, x1, y1, x2, y2, duration), 1)

        # Swipe in the navigation bar area if the device has one, when in full screen mode
        if full_screen_mode == True and (y1 >= height_with_navbar or y2 >= height_with_navbar):
        ...
        if result in failed_tag_list:
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Swiped the screen successfully", 1)
            return 'passed'
    except Exception:
...
def read_screen_heirarchy():
    ...
    try:
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Read screen heirarchy successfully", 1)
        if data:
            return data
        else:
            return False
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Read screen heirarchy unsuccessfully", 3)
        return False
...
def tap_location(data_set):
    ...
    try:
        # Tap the location (must be in list format)
        appium_driver.tap(positions)
...
def get_element_location_by_id(data_set):
    ...
    try:
        ...
        if _id == '' or action_value == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Could not find element parameter", 3)
            return 'failed'
    except Exception:
        errMsg = "Unable to parse data set"
        return CommonUtil.Exception_Handler(sys.exc_info(), None, errMsg)
...
def get_window_size(read_type=False):
    ...
    try:
        if read_type:
            # Works well at reading height in full screen mode, but Appium may complain if you work outside the boundaries it has set
            return appium_driver.find_element_by_xpath("//*[not(*)]").size
        else:
            # Read the screen size as reported by the device - this is always the safe value to work within
            return appium_driver.get_window_size()
        ...
        if Element == "failed":
            ...
            try:
                ...
                if Element.is_enabled():
                    ...
                    CommonUtil.TakeScreenShot(sModuleInfo)
                    CommonUtil.ExecLog(
                        sModuleInfo, "Successfully clicked the element with given parameters and values", 1)
                    return "passed"
                else:
                    CommonUtil.TakeScreenShot(sModuleInfo)
                    CommonUtil.ExecLog(
        ...
        if Element == "failed":
            ...
            try:
                if Element.is_enabled():
                    ...
                    action.press(Element).wait(100).release().press(
                        Element).wait(100).release().perform()
    ...
    try:
        ...
        if Element == "failed":
            ...
            try:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "Found element, but couldn't write text to it. Trying another method", 2)
    ...
    try:
        if appium_details[device_id]['type'] == 'android':
            ...
            subprocess.check_output(
                "adb %s shell input keyevent 123" % (serial), shell=True)
            subprocess.check_output("adb %s shell input keyevent %s" % (
                serial, Delet_Text), shell=True)
            # enters the string
            subprocess.check_output("adb %s shell input text '%s'" % (
                serial, text_to_enter), shell=True)  # Enter password
            time.sleep(0.5)
            result = 'passed'
...
def Android_Keystroke_Key_Mapping(keystroke, hold_key=False):
    ...
    try:
        ...
        # Task switcher / overview screen
        elif keystroke in ("app switch", "task switch", "overview", "recents"):
            key = 187
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Unsupported key event: %s" % keystroke, 3)
            return 'failed'

        if hold_key:
            # About 0.5s hold, not configurable
            appium_driver.long_press_keycode(key)
        else:
            # driver.keyevent() is depreciated
            appium_driver.press_keycode(key)
...
def iOS_Keystroke_Key_Mapping(keystroke):
    ...
    CommonUtil.ExecLog(
        sModuleInfo, "IOS key events not yet supported" % keystroke, 3)
    return 'failed'
...
def Keystroke_Appium(data_set):
    ...
    # Parse data set
    try:
        keystroke_type = data_set[0][0].replace(
            ' ', '').lower()  # "keypress" or "long press"
        keystroke_value = data_set[0][2]

        if keystroke_value == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Could not find keystroke value", 3)
            return 'failed'
    ...
    try:
        ...
        if result in passed_tag_list:
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
                sModuleInfo, "Successfully entered keystroke for the element with given parameters and values", 1)
            return "passed"
        else:
            CommonUtil.TakeScreenShot(sModuleInfo)
            CommonUtil.ExecLog(
                sModuleInfo, "Could not enter keystroke for the element with given parameters and values", 3)
            return "failed"
...
def Validate_Text_Appium(data_set):
    ...
    try:
        for each_step_data_item in data_set[0]:
            if ("parameter" in each_step_data_item[1] or each_step_data_item[1] == "element parameter") and each_step_data_item[2] == '':
...
#             print (">>>>>>>> Actual Text: %s" %actual_text_data)
            for each_actual_text_data_item in actual_text_data:
                ...
                if expected_text_data[0] in each_actual_text_data_item:
                    CommonUtil.ExecLog(
                        sModuleInfo, "Validate the text element %s using partial match." % visible_list_of_element_text, 0)
                    return "passed"
                else:
                    CommonUtil.ExecLog(
                        sModuleInfo, "Unable to validate the text element %s. Check the text element(s) in step_data(s) and/or in screen text." % visible_list_of_element_text, 3)
                    return "failed"
...
#             print (">>>>>>>> Actual Text: %s" %actual_text_data)
            # index [0] used to remove the unicode 'u' from the text string
            if (expected_text_data[0] == actual_text_data[0]):
                CommonUtil.ExecLog(
                    sModuleInfo, "Validate the text element %s using complete match." % visible_list_of_element_text, 0)
                return "passed"
            else:
                CommonUtil.ExecLog(
                    sModuleInfo, "Unable to validate the text element %s. Check the text element(s) in step_data(s) and/or in screen text." % visible_list_of_element_text, 3)
        ...
        if validation_type == "validate screen text":
            CommonUtil.ExecLog(
                sModuleInfo, ">>>>>> Expected Text: %s" % expected_text_data, 0)
#             print (">>>>>> Expected Text: %s" %expected_text_data)
            CommonUtil.ExecLog(sModuleInfo, ">>>>>>>> Actual Text: %s" %
                               visible_list_of_element_text, 0)
#             print (">>>>>>>> Actual Text: %s" %visible_list_of_element_text)
            ...
            for x in xrange(0, len(visible_list_of_element_text)):
                ...
                if (visible_list_of_element_text[x] == expected_text_data[i]):
                    CommonUtil.ExecLog(
                        sModuleInfo, "The text element '%s' has been validated by using complete match." % visible_list_of_element_text[x], 1)
                    i += 1
                    return "passed"
                else:
                    visible_elem = [
                    ...
                    expected_elem = [
                        ee for ee in expected_text_data[i].split()]
                    for elem in visible_elem:  # Validate the matching word
                        if elem in expected_elem:
                            CommonUtil.ExecLog(
                                sModuleInfo, "Validate the text element '%s' using element match." % elem, 1)
                            return "passed"
                        else:
                            CommonUtil.ExecLog(
                                sModuleInfo, "Unable to validate the text element '%s'. Check the text element(s) in step_data(s) and/or in screen text." % elem, 1)
                            return "failed"
    ...
    try:
        ...
        for line in ary:  # For each package
            if search_name.lower() in line.lower():
                package_list.append(line.replace(
                    'package', '').replace(':', ''))
        ...
        if len(package_list) == 0:
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Found more than one packages. Will use the first found. Please specify a more accurate package name. Found packages: %s" % package_list, 2)
...
def device_information(data_set):
    ...
    try:
        ...
        if shared_var == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Action's Value contains incorrect information. Expected Shared Variable, or string", 3)
            return 'failed'
    except Exception:
    ...
    try:
        if cmd == 'imei':
            ...
            # Anything else, try to figure out what it is
                ...
                elif adbOptions.is_android_connected(shared_var):
                    pass
        ...
        if shared_var != '':
            ...
            CommonUtil.ExecLog(sModuleInfo, "Saved %s [%s] as %s" % (
                cmd, str(output), shared_var), 1)
        return 'passed'
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info())
...
def set_device_password(data_set):
    ...
    try:
        ...
        if password != '':
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Password cannot be blank. Expected Value field of action row to be a PIN or PASSWORD", 3)
            return 'failed'
...
def switch_device(data_set):
    ...
    try:
        ...
        if ID != '':
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Serial number cannot be blank. Expected Value field of action row to be a serial number or UUID of the device connected via USB", 3)
            return 'failed'
...
def package_information(data_set):
    ...
    try:
        ...
        if package_name == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Full or partial package name missing. Expected Value field to contain it", 3)
            return 'failed'
    ...
    try:
        if cmd == 'package version':
            if shared_var == '':
                CommonUtil.ExecLog(
                    sModuleInfo, "Shared Variable name expected in Value field on action row", 3)
                return 'failed'
        ...
        if result in failed_tag_list or result == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Error trying to execute mobile program", 3)
            return 'failed'

        CommonUtil.ExecLog(sModuleInfo, "%s was successful" % cmd, 1)
        if shared_var != '':
            CommonUtil.ExecLog(sModuleInfo, "Value '%s' saved to Shared Variable '%s'" % (
                value, shared_var), 1)
        return 'passed'
    except Exception:
    ...
    try:
        ...
        for row in data_set:
            if str(row[1]) == 'action':
                ...
                if len(splitted) < 2:
                    CommonUtil.ExecLog(
                        sModuleInfo, "Data should be like variable_name=boolean (for example: abc=True, Here abc is the variable name and True is the boolean value)", 3)
                    return "failed"

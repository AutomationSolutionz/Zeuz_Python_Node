...
default_tag_list = ['Default', 'default',
                    'DEFAULT', 'Unchanged', 'unchanged', 'UNCHAGED']
update_tag_list = ['Update', 'update', 'UPDATE', 'Replace',
                   'replace', 'REPLACE']  # Update/replace existing element(s)
delete_tag_list = ['Delete', 'delete', 'DELETE', 'Remove',
                   'remove', 'REMOVE']  # Delete existing line or element(s)
        ...
        if (len([step_data]) != 1):  # Verifies that length of step_data greater than one
            ...
            if ((element_step_data == []) or (element_step_data in failed_tag_list)):
                ...
                returned_element = get_target_element(
                    element_step_data[0], element_step_data[1], element_step_data[2], element_step_data[3], element_step_data[4], step_data)

                # if failed to update target elements
                if ((returned_element == []) or (returned_element in failed_tag_list)):
                    CommonUtil.ExecLog(
                        sModuleInfo, "Unable to change the attribute value of '%s'" % element_step_data[0], 3)
                    return "Failed"

                elif returned_element in default_tag_list:  # if target elements are same as expected elements
                    CommonUtil.ExecLog(
                        sModuleInfo, "Nothing to change the attribute value of '%s'" % element_step_data[0], 1)
                    return "Passed"

                else:
                    CommonUtil.ExecLog(
                        sModuleInfo, "Updated the target attribute value of '%s'" % element_step_data[0], 1)
                    return "Passed"
...
def read_element(step_data):
    ...
    try:
        if (len([step_data]) != 1):  # Verifies that length of step_data greater than one
            ...
            if ((element_step_data == []) or (element_step_data in failed_tag_list)):
                CommonUtil.ExecLog(
                    sModuleInfo, "Unable to get the file path of: '%s'" % element_step_data[0], 3)
                return "Failed"

            else:
                # Function to get the XML file tree
                returned_element = get_file_tree(element_step_data[0])
                CommonUtil.ExecLog(sModuleInfo, "File tree of: %s" %
                                   (element_step_data[0]), 1)
                CommonUtil.ExecLog(sModuleInfo, "%s" %
                                   ET.tostring(returned_element[0]), 1)
                return "Passed"
...
def get_element_step_data(step_data):
    ...
    try:
        ...
        if (len(step_data) >= 1):
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Data set incorrect. Please provide accurate data set(s) information.", 3)
            return "Failed"

        returned_data = (file_path, target_parameter,
                         target_value, action_parameter, action_value)
...
def get_target_element(file_path, target_parameter, target_value, action_name, action_value, step_data):
    ...
    try:
        ...
        returned_target_element = update_target_element(
            file_path, file_tree[1], matching_elements, target_parameter, target_value, action_name, action_value)
...
def update_target_element(file_path, doc, matching_elements, target_attrib, target_value, action_name, action_value):
    ...
    try:
        ...
        if current_value == action_value:
            CommonUtil.ExecLog(
                sModuleInfo, ">>> The current attribute value '%s' is already set to the target value." % current_value, 1)
            return "Default"

        elif current_value != action_value:
            CommonUtil.ExecLog(
                sModuleInfo, ">>> The current attribute value is: '%s'" % current_value, 1)

        else:
            CommonUtil.ExecLog(
                sModuleInfo, ">>> Unable to find the target attribute. Please check the data set(s).", 1)
        ...
        if action_name == "update":
            ...
            CommonUtil.ExecLog(
                sModuleInfo, ">>> Unable to find the action name. Please check the data set(s).", 1)

        # Function the write the user provided action value in the XML file
        returned_action_value = update_action_value(file_path, doc)
        CommonUtil.ExecLog(
            sModuleInfo, ">>> The current attribute value updated to the expected value '%s'" % action_value, 1)

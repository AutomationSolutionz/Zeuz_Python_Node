    ...
    try:
        ...
        if os.path.isdir(folderPath):
            if forced == False:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "Folder already exists. Not doing", 1)
                return "passed"
...
def MoveFile(file_to_be_moved, new_name_of_the_file):
    ...
    try:
        CommonUtil.ExecLog(sModuleInfo, "Moving file %s to %s" %
                           (file_to_be_moved, new_name_of_the_file), 0)
...
def MoveFolder(folder_to_be_moved, new_name_of_the_folder):
    ...
    try:
        CommonUtil.ExecLog(sModuleInfo, "Moving folder from %s to %s" % (
            folder_to_be_moved, new_name_of_the_folder), 1)
...
def UnZip(file_to_be_unzipped, location_where_to_unzip):
    ...
    try:
        CommonUtil.ExecLog(sModuleInfo, "Unzipping file %s to %s" % (
            file_to_be_unzipped, location_where_to_unzip), 1)
        if os.path.isfile(file_to_be_unzipped):  # If zip file exists
            # If destination directory does not exist, create it
            if not os.path.isdir(location_where_to_unzip):
                CommonUtil.ExecLog(
                    sModuleInfo, "Directory doesn't exist, creating", 1)
                # Create directory, but if it fails, exit
                if CreateFolder(location_where_to_unzip) in failed_tag_list:
                    CommonUtil.ExecLog(
...
def CompareFile(file_to_be_compared1, file_to_be_compared2):
    ...
    try:
        CommonUtil.ExecLog(sModuleInfo, "Comparing %s with %s" %
                           (file_to_be_compared1, file_to_be_compared2), 1)
...
def ZipFile(source, destination):
    ...
    try:
        CommonUtil.ExecLog(sModuleInfo, "Zipping file %s to %s" %
        ...
        if os.path.exists(source):  # Verify source exists
                    ...
                    # Write directory and all contents recursively to archive
                        ...
                        for root, subdirs, files in os.walk(source):
                            for subdir in subdirs:
                                ...
                                z.write(os.path.join(root, subdir))
            ...
            if os.path.isfile(destination):
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not create Zip archive", 0)
                return "failed"
        else:
...
def DeleteFile(sFilePath):
    ...
    try:
        ...
        if os.path.isfile(sFilePath):
            ...
            if os.path.isfile(sFilePath):
                CommonUtil.ExecLog(
                    sModuleInfo, "file exists... delete function is not done properly", 3)
                return "failed"
            else:
                CommonUtil.ExecLog(
                    sModuleInfo, "file doesn't exist... delete function is done properly", 0)
                return "passed"
...
def DeleteFolder(sFolderPath):
    ...
    try:
        ...
        if os.path.isdir(sFolderPath):
            ...
            if os.path.isdir(sFolderPath):
                CommonUtil.ExecLog(
                    sModuleInfo, "folder exists... delete function is not done properly", 3)
                return "failed"
            else:
                CommonUtil.ExecLog(
                    sModuleInfo, "folder doesn't exist... delete function is done properly", 0)
                return "passed"
        else:
            CommonUtil.ExecLog(
...
def find(sFilePath):  # !!!Needs to be updated to either return true/false or actually try to find a file in a file system
    ...
    try:
        CommonUtil.ExecLog(
            sModuleInfo, "Finding file %s is complete" % sFilePath, 1)
...
def empty_trash(trash_path):  # !!! Change this so that it's a general recursive delete of all files and sub-directories. shouldn't limit to trash can, then have a smaller empty trash function
    ...
    try:
        CommonUtil.ExecLog(
            sModuleInfo, "Starting to empty trash %s" % trash_path, 1)
        ...
        for dir, sub_dirs, files in os.walk(trash_path):
            if not files:
                CommonUtil.ExecLog(
        ...
        if flag == 1:
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "------Trash is empty already------", 1)
            return "passed"  # return "failed" if trash is already cleared

        CommonUtil.ExecLog(
            sModuleInfo, "Emptying trash %s is complete" % trash_path, 1)
...
def copy_folder(src, dest):
    ...
    try:
        CommonUtil.ExecLog(
            sModuleInfo, "Coping folder %s to %s" % (src, dest), 0)
        ...
        if os.path.isdir(dest):
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "folder doesn't exist... copy function is not done properly", 3)
            return "failed"
...
def copy_file(src, dest):  # !!!merge with copy_folder, just check if src is a file or not
    ...
    try:
        CommonUtil.ExecLog(
            sModuleInfo, "Coping file %s to %s" % (src, dest), 0)
        ...
        if os.path.isfile(dest):
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "file doesn't exist... copy function is not done properly", 3)
            return "failed"
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info())
    ...
    try:
        ...
        if flag == 1:
            result = winshell.recycle_bin().empty(
                confirm=False, show_progress=False, sound=False)  # Empty Trash
...
def download_file_using_url(file_url, location_of_file):
    ...
    try:
        ...
        file_name = os.path.join(location_of_file, str(
            file_url.split("/")[-1:][0]))  # complete file location
        CommonUtil.ExecLog(
            sModuleInfo, "Path of file to download: %s" % file_name, 0)
        with open(file_name, "wb") as f:  # Open new binary file
            # Grab chunk of received data
            for chunk in r.iter_content(chunk_size=1048576):
                if chunk:
                    f.write(chunk)  # Write chunk of data to disk
...
def Copy_File_or_Folder(step_data):
    ...
    try:
        if _platform == "linux" or _platform == "linux2" or _platform == "darwin":
            ...
            from_path = str(step_data[0][2]).strip()
            if from_path[0] == "/":
            ...
            # location of the file/folder to be copied
            from_path = raw(str(step_data[0][2]).strip())
            # location where to copy the file/folder
            to_path = os.path.join(get_home_folder(), raw(
                str(step_data[1][2]).strip()))
        # get if it is file/folder to copy
        ...
        if file_or_folder.lower() == 'file':
            ...
            if result in failed_tag_list:
                ...
                CommonUtil.ExecLog(sModuleInfo, "Folder '%s' copied to the destination '%s' successfully" % (
                    from_path, to_path), 1)
                return "passed"
        else:
            CommonUtil.ExecLog(
...
def Unzip_File_or_Folder(step_data):
    ...
    try:
        ...
        if from_path not in file_attachment and os.path.exists(os.path.join(get_home_folder(), from_path)) == False:
            CommonUtil.ExecLog(
                sModuleInfo, "Could not find file attachment called %s, and could not find it locally" % from_path, 3)
            return 'failed'
        if from_path not in file_attachment:
            from_path = os.path.join(get_home_folder(), from_path)
            if _platform == "linux" or _platform == "linux2" or _platform == "darwin":
        ...
        if result in failed_tag_list:
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "'%s' is unzipped to '%s' successfully" % (from_path, to_path), 1)
            return "passed"
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info())
...
def Delete_File_or_Folder(data_set):
    ...
    try:
        ...
        for row in data_set:
            ...
            elif row[1] in ('path', 'element paraneter') and filename == '':
                filename = row[2].strip()

        if filename == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Could not find filename or path to file in Value field of action line", 3)
            return 'failed'
    ...
    try:
        ...
        if os.path.exists(filename) == False:
            ...
            tmp = os.path.join(get_home_folder(), tmp)
            # Check if file is in home directory, or path is partially specified, we'll complete it with the home directory
            if os.path.exists(tmp) == False:
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not find file in attachments, home directory or in the local directory: %s" % filename, 3)
                return 'failed'
            else:
        ...
        if os.path.isfile(filename):
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "File/directory specified does exist, but is neither a file nor a directory. It could not be deleted", 3)
            return 'failed'
...
def Create_File_or_Folder(step_data):
    ...
    try:
        if _platform == "linux" or _platform == "linux2" or _platform == "darwin":
            ...
            # get if it is file/folder to create
...
def Find_File(step_data):
    ...
    try:
        ...
        if file_or_folder.lower() == 'file':
            ...
            if result in failed_tag_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not find file '%s'" % (path), 3)
                return "failed"
            else:
                CommonUtil.ExecLog(
                    sModuleInfo, "File '%s' is found" % (path), 1)
                return "passed"
    ...
    try:
        if _platform == "linux" or _platform == "linux2" or _platform == "darwin":
            ...
            if result in failed_tag_list:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "trash is cleared '%s'" % (path), 1)
                return "passed"
        elif _platform == "win32":
            # windows
            ...
            if result in failed_tag_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not empty recycle bin ", 3)
                return "failed"
            else:
    ...
    try:
        if _platform == "linux" or _platform == "linux2" or _platform == "darwin":
            ...
            if name in failed_tag_list:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "user name is '%s'" % (name), 1)
                return "passed"
        elif _platform == "win32":
            # windows
            CommonUtil.ExecLog(sModuleInfo, "windows", 1)
...
def Add_Log(step_data):
    ...
    CommonUtil.ExecLog(sModuleInfo, "Function start", 0)
    try:
        # get the log level info from the step data
        log_info = str(step_data[0][0]).strip()
...
def Calculate(step_data):
    ...
    try:
        # Prepare data
        statement = str(step_data[0][2]).strip().replace(
        ...
        CommonUtil.ExecLog(sModuleInfo, "Calculation: %s = %s" %
                           (math_string, result), 1)
...
def Run_Command(data_set):
    ...
    try:
        ...
        if len(commands) == 0:
            CommonUtil.ExecLog(
                sModuleInfo, "No commands specified. Expected at least one row to contain 'run command' in the Field, and the command to execute in the Value field", 3)
            return 'failed'
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info(), None, "Error parsing data set")
    ...
    try:
        ...
        h = subprocess.Popen(delim.join(commands), shell=True,
        ...
        CommonUtil.ExecLog(sModuleInfo, "Command output: %s" %
                           output, 1)  # Write output to log
        if shared_var:
...
def Get_Home_Directory(data_set):
    ...
    try:
        # Get shared variable name from Value on action row
        home_dir = data_set[0][2]
    except:
        return CommonUtil.Exception_Handler(sys.exc_info())
    ...
    try:
        ...
        if path in failed_tag_list:
            ...
            return 'failed'
        CommonUtil.ExecLog(
            sModuleInfo, "Home Directory Path is '%s'" % (path), 1)

        # Store home directory in shared variable
        Shared_Resources.Set_Shared_Variables(home_dir, path)
        return 'passed'
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info())
...
def Get_Current_Desktop(step_data):
    ...
    try:
        ...
        path = os.path.join(get_home_folder(), 'Desktop')

        if path in failed_tag_list:
            CommonUtil.ExecLog(
                sModuleInfo, "Could not find desktop '%s'" % (path), 3)
            return "failed"
...
def Get_Current_Documents(step_data):
    ...
    try:
        # concate home folder path with "/Desktop"
        path = os.path.join(get_home_folder(), 'Documents')
    ...
    try:
        ...
        if file_or_folder.lower() == 'file':
            ...
            if result in failed_tag_list:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "File '%s' created" % (path), 1)
                return "passed"
...
def Compare_File(step_data):
    ...
    try:
        if _platform == "linux" or _platform == "linux2" or _platform == "darwin":
            ...
            # print  from_path
            # location of file path to be compared
            to_path = raw(str(step_data[1][2]).strip())
        ...
        if to_path in file_attachment:
            # In file is an attachment, get the full path
            to_path = file_attachment[to_path]
        ...
        if file_or_folder.lower() == 'file':
            ...
            if result in failed_tag_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Files %s and %s are not equal" % (to_path, from_path), 3)
                return "failed"
            else:
                CommonUtil.ExecLog(
...
def Rename_File_or_Folder(step_data):
    ...
    try:
        if _platform == "linux" or _platform == "linux2" or _platform == "darwin":
            ...
            from_path = get_home_folder() + str(step_data[0][2]).strip()
            # location where to rename the file/folder
            to_path = get_home_folder() + str(step_data[1][2]).strip()
        elif _platform == "win32":
            # location of the file/folder to be renamed
            from_path = raw(str(step_data[0][2]).strip())
            # location where to rename the file/folder
        ...
        # get if it is file/folder to rename
        file_or_folder = str(step_data[2][2]).strip()
        if file_or_folder.lower() == 'file':
            ...
            if result in failed_tag_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not rename file '%s' to '%s'" % (from_path, to_path), 3)
                return "failed"
            else:
                CommonUtil.ExecLog(
                    sModuleInfo, "File '%s' renamed to '%s' successfully" % (from_path, to_path), 1)
                return "passed"
        elif file_or_folder.lower() == 'folder':
            ...
            if result in failed_tag_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not rename folder '%s' to '%s'" % (from_path, to_path), 3)
                return "failed"
            else:
...
def Zip_File_or_Folder(data_set):
    ...
    # Zip file
    try:
        # Perform zip on file or directory
        result = ZipFile(source, destination)
        ...
        if result in failed_tag_list:
            ...
            CommonUtil.ExecLog(sModuleInfo, "File '%s' zipped to '%s' successfully" % (
...
def Move_File_or_Folder(step_data):
    ...
    try:
        ...
        # get if it is file/folder to move
        file_or_folder = str(step_data[2][2]).strip()
        if file_or_folder.lower() == 'file':
            ...
            if result in failed_tag_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not move file '%s' to '%s'" % (from_path, to_path), 3)
                return "failed"
            else:
                CommonUtil.ExecLog(
                    sModuleInfo, "File '%s' moved to '%s' successfully" % (from_path, to_path), 1)
                return "passed"
        elif file_or_folder.lower() == 'folder':
            ...
            if result in failed_tag_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Could not move folder '%s' to '%s'" % (from_path, to_path), 3)
                return "failed"
            else:
                CommonUtil.ExecLog(
                    sModuleInfo, "Folder '%s' moved to '%s' successfully" % (from_path, to_path), 1)
                return "passed"
        else:
            CommonUtil.ExecLog(
                sModuleInfo, "The information in the data-set(s) are incorrect. Please provide accurate data set(s) information.", 3)
...
def Upload(step_data):
    ...
    try:
        if _platform == "linux" or _platform == "linux2" or _platform == "darwin":
            # linux
            CommonUtil.ExecLog(sModuleInfo, "linux", 1)
            from_path = get_home_folder(
            ) + str(step_data[0][2]).strip()  # location of the file/folder to be copied\
            temp_ini_file = get_home_folder() + "/Desktop/AutomationLog/temp_config.ini"
            list = from_path.split("/")
            # location where to copy the file/folder
            to_path = ConfigModule.get_config_value(
                'sectionOne', 'test_case_folder', temp_ini_file) + "/" + list[len(list) - 1]

        elif _platform == "win32":
            # windows
            CommonUtil.ExecLog(sModuleInfo, "windows", 1)
            # location of the file/folder to be copied
            from_path = raw(str(step_data[0][0]).strip())
        ...
        result = copy_file(from_path, to_path)
        if result in failed_tag_list:
            CommonUtil.ExecLog(sModuleInfo, "Could not copy file '%s' to the log uploader '%s'" % (
                from_path, to_path), 3)
            return "failed"
        else:
            CommonUtil.ExecLog(sModuleInfo, "File '%s' copied to the log uploader '%s' successfully" % (
                from_path, to_path), 1)
            return "passed"
...
temp_config = os.path.join(os.path.join(get_home_folder(), os.path.join(
...
def TakeScreenShot(step_data):
    ...
    try:
        ...
        # location of the folder where the screen shot should be saved
        ...
        if os.name == 'posix':
            ...
            full_location = ImageFolder + os.sep + \
                TimeStamp("utc") + "_" + ImageName + '.png'
            CommonUtil.ExecLog(
                sModuleInfo, "file location is %s" % full_location, 1)
            ...
            try:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, 'could not import python package needed for screenshot...installing package "gi"', 1)
            ...
            if img:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "unable to take screenshot...", 1)
            ...
            from PIL import Image
            # location of the file where the screen shot should be saved
            path = ImageFolder + os.sep + \
                TimeStamp("utc") + "_" + ImageName + ".jpg"
            img = ImageGrab.grab()
            basewidth = 1200
            ...
            img.save(path, 'JPEG')
            CommonUtil.ExecLog(
                sModuleInfo, 'screenshot saved as: "%s"' % path, 1)
...
def Sleep(step_data):
    ...
    try:
        if ((len(step_data) != 1)):
            ...
            seconds = int(tuple[2])
            CommonUtil.ExecLog(
                sModuleInfo, "Sleeping for %s seconds" % seconds, 1)
            time.sleep(seconds)
            return "passed"
            # return result
    except Exception:
    ...
    try:
        if(step_data[0][1] == 'value'):
            result = Shared_Resources.Set_Shared_Variables(
                step_data[0][2], int(step_data[1][2]))  # setting the shared variables
        else:
            result = Shared_Resources.Set_Shared_Variables(
                step_data[0][2], step_data[1][2])  # setting the shared variables
        if result in failed_tag_list:
            CommonUtil.ExecLog(
...
def Download_file(data_set):
    ...
    try:
        ...
        if url == '':  # Make sure we have a URL
            CommonUtil.ExecLog(
                sModuleInfo, "Expected Field to contain 'url' and Value to contain a valid URL to a file", 3)
            return 'failed'
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info(), None, "Error parsing data set")
    ...
    try:
        ...
        if file_name in failed_tag_list:
            CommonUtil.ExecLog(sModuleInfo, "Failed to save file from (%s) to disk (%s)" % (
                url, file_location), 3)
            return "failed"
        else:
            CommonUtil.ExecLog(
            ...
            if shared_var:
                ...
                Shared_Resources.Set_Shared_Variables(shared_var, file_name)
            return "passed"
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info(), None, "Error downloading file")
...
def Download_File_and_Unzip(data_set):
    ...
    try:
        ...
        if url == '':  # Make sure we have a URL
            ...
            return 'failed'
        if unzip_location == '':
            # Set unzip location to Downloads by default if omited
            unzip_location = file_location
    except Exception:
        return CommonUtil.Exception_Handler(sys.exc_info(), None, "Error parsing data set")
    ...
    try:
        ...
        if result in failed_tag_list:
            CommonUtil.ExecLog(sModuleInfo, "Failed to unzip %s to %s" % (
                file_name, unzip_location), 3)
            return "failed"
        else:
            CommonUtil.ExecLog(
    ...
    try:
        ...
        for row in data_set:
            ...
            if row[1] == 'element parameter':
                if row[0] == 'replace all':
                    ...
                    elif row[2].lower().strip() == 'false' or row[2].lower().strip() == 'no':
                        replace_all = False
                    elif row[2] != '':
                        CommonUtil.ExecLog(
                            sModuleInfo, "Unknown Value for element parameter 'replace_all'. Should be true or false.", 3)
                        return 'failed'
                    ...
                    # case_insensitive
                        ...
                        CommonUtil.ExecLog(
                            sModuleInfo, "Unknown Value for element parameter 'case_sensitive'. Should be true or false.", 3)
                        return 'failed'
                    ...
                    if row[2] != '':
                        try:
                            ...
                            CommonUtil.ExecLog(
                                sModuleInfo, "Unknown Value for element parameter 'dictionary'. Should be a string representation of python dictionary.", 3)
                            return 'failed'
                    else:
                        CommonUtil.ExecLog(
                            sModuleInfo, "Unknown Value for element parameter 'dictionary'. Should be a string representation of python dictionary.", 3)
                    ...
                    # substring should be replaced to this string
                    new_string = row[2].strip()
        ...
        if find_value != '' and replace_with != '' and var_name != '':
            try:
                ...
                CommonUtil.ExecLog(sModuleInfo, "Replace '%s' with '%s' in variable %s" % (
                    find, replace_with, var_name), 1)
                Shared_Resources.Set_Shared_Variables(var_name, value)
                CommonUtil.ExecLog(
                    sModuleInfo, "Value '%s' is save in variable %s" % (value, var_name), 1)
                return "passed"
            except Exception, e:
                CommonUtil.ExecLog(
...
def Change_Value_ini(data_set):
    ...
    try:
        ...
        for row in data_set:
            ...
            if row[1] == 'path':
                # name of the file where the change should be made
                file_name = row[2]
            if row[1] == 'value':
                # name of the section where th change should be made
                section_name = row[0]
                # name of the line where value should be changed
                line_name = row[2]
    ...
    try:
        ...
        if result:
            ...
            CommonUtil.ExecLog(sModuleInfo, "Error updating %s with %s in section %s" % (
                line_name, new_expected_value_of_line, section_name), 3)
        return "failed"
...
def Add_line_ini(data_set):
    ...
    try:
        ...
        if result:
            ...
            CommonUtil.ExecLog(sModuleInfo, "Error updating %s with %s in section %s" % (
                line_name, value_of_line, section_name), 3)
        return "failed"
...
def Read_line_name_and_value(data_set):
    ...
    try:
        ...
        if save_line_name_value == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Could not find the name where to save line name and value in shared variables for this action", 3)
            return 'failed'
    except:
        return CommonUtil.Exception_Handler(sys.exc_info(), None, "Error parsing data set")
    try:
    ...
    try:
        ...
        for each in step_data:
            ...
            if (each[1] == "action" or each[1] == "conditional action"):
                CommonUtil.ExecLog(
                    sModuleInfo, "Not a part of element step data", 2)
                continue
            else:
                element_step_data.append(each)
...
def Validate_Path_Step_Data(step_data):
    ...
    try:
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Could not find the new page element requested.  Error: %s" % (Error_Detail), 3)
        return "failed"
...
def pattern_matching(dataset):
    ...
    try:
        ...
        for row in dataset:
            ...
            elif row[0].lower().strip() == 'variable':
                shared_var = row[2].strip()
    ...
    try:
        ...
        if m == [] or m == None:
            CommonUtil.ExecLog(
                sModuleInfo, "Pattern did not produce a match", 3)
            return 'failed'
        else:
            # !!! Idealy would save the entire list, but we need a way to access a single element
            Shared_Resources.Set_Shared_Variables(shared_var, m[0])
            CommonUtil.ExecLog(
                sModuleInfo, "Pattern matched: %s - Saved to Shared variable: %s" % (str(m[0]), shared_var), 1)
            return 'passed'
    except:
...
def save_substring(data_set):
    ...
    try:
        ...
        if Shared_Resources.Test_Shared_Variables(from_var):
            ...
            from_var = Shared_Resources.Get_Shared_Variables(from_var)
        else:
            CommonUtil.ExecLog(
        ...
        if from_index == -1:
            ...
            if to_index == -1:
                try:
                    ...
                    CommonUtil.ExecLog(
                        sModuleInfo, "Can't get substring.Index out of range for string '%s'" % from_var, 3)
                    return "failed"
            else:
                try:
...
def extract_number(data_set):
    ...
    try:
        from_var = ''  # the varibale from which string will be copied
        to_var = ''  # the variable wher estring will be saved
        # index of the digit for a string "hello123 2.5 67" if index is 2(starting from 1) we will get 2.5
        index = 0
        rounded_number = ''
        digit = '-1'
        extracted_number = ''
        ...
        if index < 0:
            ...
            try:
                ...
                if digit < 1:
                    ...
                    rounded_number = int(
                        (round(float(extracted_number) / digit))) * digit
                else:
                    c = 0
                    for i in range(1, 20):
                        digit *= 10
                    ...
                    if digit != 1:
                        CommonUtil.ExecLog(
                            sModuleInfo, "Can't round number. Incorrect digit '%s' given" % digit, 3)
                        return "failed"
                    rounded_number = round(float(extracted_number), c)
            except:
                CommonUtil.ExecLog(
                    sModuleInfo, "Can't extract digit. Index out of range for string '%s'" % from_var, 3)
                return "failed"

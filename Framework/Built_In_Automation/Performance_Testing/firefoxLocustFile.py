...
def get_stop_timeout():
    file = open(os.getcwd() + os.sep + 'Built_In_Automation' + os.sep +
                'Performance_Testing' + os.sep + 'locustFileInput.txt', 'r')
...
class LocustUserBehavior(TaskSet):
    ...
    def runLocust(self):
        ...
        print is_linked
        send_log_file_only_for_fail = ast.literal_eval(
            str(file.readline()).strip())
        print send_log_file_only_for_fail
        file.close()
        MainDriverApi.run_test_case(TestCaseID, sModuleInfo, run_id, driver_list, final_dependency,
                                    final_run_params, temp_ini_file, is_linked, send_log_file_only_for_fail, True, self.client)

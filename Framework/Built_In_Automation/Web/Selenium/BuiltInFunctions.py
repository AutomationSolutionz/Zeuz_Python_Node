    ...
    if not wait_clickable:
        ...
        while True:
            try:
                ...
                if log_flag:
                    CommonUtil.ExecLog(sModuleInfo,
                                       "The Element is overlapped. Waiting %s seconds max for the element to become clickable" % wait_clickable,
                                       2)
                    log_flag = False
    ...
    try:
        ...
        if not driver_id:
            ...
            try:
                ...
                CommonUtil.ExecLog(sModuleInfo, "Unable to tear down driver_id='%s'. may already be killed" % driver_id,
                                   2)
...
def slider_bar(data_set):
    ...
    try:
        ...
        if value not in range(0, 100):
            CommonUtil.ExecLog(sModuleInfo,
                               "Failed to parse data/locate element. You must provide a number between 0-100", 3)
            return "zeuz_failed"
        Element = LocateElement.Get_Element(data_set, selenium_driver)
        if Element == "zeuz_failed":
            CommonUtil.ExecLog(sModuleInfo, "Could not find the element", 3)

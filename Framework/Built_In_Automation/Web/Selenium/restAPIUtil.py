    ...
    try:
        if ((rest_call_type == "") or (url == "")):
            ...
            if response.status_code == 200:
                CommonUtil.ExecLog(
                    sModuleInfo, "Successful REST request. Status Code: %s" % response.status_code, 1)
                if payload_type == "json":
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "Error in REST request. Status Code: %s" % response.status_code, 3)
...
def _response_Validation(payload_type, response, extraction_data=False):
    ...
    try:
        ...
        # For payload body type: json
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Specific validation not provided. Will need to be implemented in the future.", 2)
...
def Exception_Info(sModuleInfo, errMsg):
    ...
    CommonUtil.ExecLog(sModuleInfo, errMsg + ".  Error: %s" %
                       (Error_Detail), 3)
    return "failed"

...
def verify_step_data(step_data):
    ...
    try:
        ...
        for data_set in step_data:
            ...
            for row in data_set:
                if len(row[0]) == 0:
                    CommonUtil.ExecLog(sModuleInfo, "Field for data set %d cannot empty: %s" % (
                        data_set_index, str(row)), 3)
                    return 'failed'
                elif len(row[1]) == 0:
                    CommonUtil.ExecLog(
                        sModuleInfo, "Sub-Field for data set %d cannot empty: %s" % (data_set_index, str(row)), 3)
                    return 'failed'
                # Check against list of allowed Sub-Fields
                elif str(row[1]).lower().strip() not in action_support:
                    # !!! Temporary until module handling is all moved into it's own function
                    if 'action' not in row[1] and str(row[1]).strip().lower() not in programming_logic_keywords:
                        CommonUtil.ExecLog(
                            sModuleInfo, "Sub-Field for data set %d contains invalid data: %s" % (data_set_index, str(row)), 3)
                        return 'failed'
                ...
                if 'action' in row[1]:  # Only apply to actions rows
                    ...
                    for action_index in actions:
                        ...
                        if actions[action_index]['module'] in row[1]:
                            # Save this for the "Field" check below
                            module_name = actions[action_index]['module']
                            module_test = True  # Flag it's good
                            break
                    if module_test == False:
                        CommonUtil.ExecLog(
                            sModuleInfo, "Sub-Field for data set %d is missing a module name: %s" % (data_set_index, str(row)), 3)
                        return 'failed'
                ...
                elif 'custom' in row[1]:
                    continue
                # Only apply to actions rows
                elif 'action' in row[1] and 'conditional' not in row[1]:
                    for action_index in actions:
                        # If one of the action names in the Field
                        if (actions[action_index]['name'] == row[0] and actions[action_index]['module'] == module_name) or (actions[action_index]['name'] == row[0] and actions[action_index]['module'] == 'common') or str(row[0]).startswith('%|'):
                            field_text = True  # Flag it's good
                            break
                    if field_text == False:
                        CommonUtil.ExecLog(sModuleInfo, "Field for data set %d contains invalid data: %s" % (
                            data_set_index, str(row)), 3)
                        return 'failed'
                ...
                # Make sure recall result row contains valid commands and shared variables
                    ...
                    if row[0].strip().lower() not in ('store', 'recall'):
                        CommonUtil.ExecLog(
                            sModuleInfo, "Field for data set %d contains invalid data: %s - expected either 'recall' or 'store'" % (data_set_index, str(row)), 3)
                        return 'failed'
                    elif row[0].strip().lower() == 'recall' and '%|' not in row[2].strip():
                        CommonUtil.ExecLog(
                            sModuleInfo, "Field for data set %d contains invalid data: %s - expected Shared Variable in the proper format. Eg: %%|VAR|%%" % (data_set_index, str(row)), 3)
                        return 'failed'

            # Make sure each data set has an action row
            if action == False:
                CommonUtil.ExecLog(
                    sModuleInfo, "Data set %d is missing an action line, or it's misspelled" % data_set_index, 3)
                return 'failed'
...
def adjust_element_parameters(step_data, platforms):
    ...
    if sr.Test_Shared_Variables('dependency') == False:  # No dependency at all
        if check_action_types('Mobile', step_data) == True:
            CommonUtil.ExecLog(
                sModuleInfo, "No dependency set - functions may not work properly if step data contains platform names", 3)
            return 'failed'
        else:
        ...
        if 'Mobile' not in dependency:  # We have a dependency, but not a mobile, so we don't need to do anything
            ...
            if check_action_types('Mobile', step_data) == False:
                CommonUtil.ExecLog(sModuleInfo, "Not a mobile Test Case", 0)
                return step_data
            else:  # Mobile actions in step data
                CommonUtil.ExecLog(
                    sModuleInfo, "Mobile (Appium) actions found in Step Data, but no Mobile dependency set", 3)
        ...
        for row in data_set:  # For each row of the data set
            ...
            for id_adj_row in data_set:  # Find if this is an appium test step
                if 'appium' in id_adj_row[1]:  # Yes, so adjust
                    ...
                    if new_row[0] == 'id' and dependency['Mobile'].lower() == 'android':
                        new_row[0] = 'resource-id'
                        new_row[2] = '*'+new_row[2]
                    elif new_row[0] == 'id' and dependency['Mobile'].lower() == 'ios':
                        # If user specifies id, they likely mean "resource-id"
                        new_row[0] = 'accessibility id'
                    # If user specifies id, they likely mean "resource-id"
                    elif new_row[0] == 'text or name' and dependency['Mobile'].lower() == 'android':
                        new_row[0] = 'text'
            ...
            if dependency['Mobile'].lower() in new_row[1]:
                new_row[1] = new_row[1].replace(dependency['Mobile'].lower(), '').replace(
                    '  ', ' ').strip()  # Remove word and clean up spaces
                # Append list as tuple to data set list
                new_data_set.append(tuple(new_row))
    ...
    # Return cleaned step_data that contains only the element paramters we are interested in
            ...
            for i in actions:
                for j in actions[i]:  # For each entry in the sub-dictionary
                    if actions[i]['module'] == 'common' and actions[i]['name'] == action_name:
                        ...
                        # Save screenshot
                        screenshot = actions[i]['screenshot']
                        # Return module and function name
                        return module, function, original_module, screenshot

            for i in actions:  # For each dictionary in the dictionary
                for j in actions[i]:  # For each entry in the sub-dictionary
                    # Module and action name match
                    if actions[i]['module'] == module and actions[i]['name'] == action_name:
                        function = actions[i]['function']  # Save function
                        # Save screenshot
                        screenshot = actions[i]['screenshot']
...
def step_result(data_set):
    ...
    if action_value in failed_tag_list:  # Convert user specified pass/fail into standard result
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Step Result action has invalid VALUE", 3)
        return 'failed'
        ...
        for i in range(timeout_duration):
            ...
            if wait_for_element_to_disappear == False:  # Wait for it to appear
                if Element not in failed_tag_list:  # Element found
                    ...
                    CommonUtil.ExecLog(
                        sModuleInfo, "Element does not exist. Sleep and try again - %d" % i, 0)
                ...
                if Element in failed_tag_list:  # Element removed
                    ...
                    CommonUtil.ExecLog(
                        sModuleInfo, "Element still exists. Sleep and try again - %d" % i, 0)
...
def Save_Text(data_set):
    ...
    try:
        ...
        for row in data_set:
            if row[1] == 'action':
                # Save action Value as the shared variable name
                variable_name = row[2]
        if variable_name == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Missing variable name to save text as from Value field on action line", 3)
            return 'failed'
    except Exception:
    ...
    try:
        ...
        for each_text_item in list_of_element_text:  # For each line of text
            if each_text_item != "":
                ...
                tmp = [c for c in each_text_item if 0 < ord(
                    c) < 127]  # Strip any binary characters
                # Append each line into one string
                visible_list_of_element_text += ''.join(tmp)
...
def Save_Current_Time(data_set):
    ...
    for each in data_set:
        if each[1] == 'element parameter':
            ...
            variable_value = str(months[now.month])+" "+str(now.day)+", "+str(
                now.year)+", "+str(now_hour)+":"+str(now.minute)+" "+time
...
def insert_list_into_another_list(data_set):
    ...
    try:
        ...
        if ";" in str(tmp[1]):  # direct initialization parent_list = [[a,b,c],[x,y,z]]
            ...
            if "|;|" in str(tmp[1]):
                parent_separator = "|;|"
            parent_list_splitted_by_semicolon = str(
                tmp[1]).strip().split(parent_separator)
            ...
            for child_list_name in all_child_list_names:
                if not sr.Test_Shared_Variables(child_list_name):
                    CommonUtil.ExecLog(
                        sModuleInfo, "List named %s not found in shared variables" % child_list_name, 3)
                    return "failed"
...
def insert_dict_into_another_dict(data_set):
    ...
    try:
        # Parse data set
        # Get key and value from Value field and clean them
        tmp = data_set[0][2].replace(' ', '').strip()
        tmp = tmp.split('=')  # Get variable name
        ...
        for child_dict_name in all_child_dict_names:
            ...
            if not sr.Test_Shared_Variables(dict_name):
                CommonUtil.ExecLog(
                    sModuleInfo, "Dict named %s not found in shared variables" % dict_name, 3)
                return "failed"
...
def sequential_actions_settings(data_set):
    ...
    try:
        ...
        shared_var = tmp.split('=')[0].strip().lower()
        # Retrieve value for variable
        value = tmp.replace(shared_var + '=', '').strip()

        # Verify this is a real variable (should be set somewhere else)
        if not sr.Test_Shared_Variables(shared_var):
            CommonUtil.ExecLog(
                sModuleInfo, "The variable name specified (%s) is not a valid Sequential Action variable" % str(shared_var), 3)
            return 'failed'

        # Save variable - all functions that use this variable will now use the new value
        CommonUtil.ExecLog(sModuleInfo, "Changing Sequential Action setting of %s from %s to %s" % (
            str(shared_var), str(sr.Get_Shared_Variables(shared_var)), str(value)), 1)
...
def get_server_variable_and_wait(data_set):
    ...
    try:
        ...
        dict = {}
        CommonUtil.ExecLog(
            sModuleInfo, "Waiting for server variable '%s'" % key, 1)
        while i <= wait_time:
...
def start_timer(data_set):
    ...
    try:
        # Parse data set
        try:
            # Get no. of seconds for timer
            seconds = int(str(data_set[0][2]).strip())
        except:
            seconds = 0
...
def wait_for_timer(data_set):
    ...
    try:
        ...
        CommonUtil.ExecLog(
            sModuleInfo, "Will wait for %d seconds" % sleep_time, 1)
...
def create_3d_list(data_set):
    ...
    try:
        ...
        if ";;" in str(tmp[1]):
            ...
            if "|;;|" in str(tmp[1]):
                major_parent_separator = "|;;|"
            a_3d_list_splitted_by_semicolon = str(
                tmp[1]).strip().split(major_parent_separator)
            for each_2d_list in a_3d_list_splitted_by_semicolon:
                ...
                if ";" in str(each_2d_list):
                    ...
                    if "|;|" in str(each_2d_list):
                        parent_separator = "|;|"
                    parent_list_splitted_by_semicolon = str(
                        each_2d_list).strip().split(parent_separator)
                    for each_split in parent_list_splitted_by_semicolon:
                        separator = ","
                        if "|,|" in each_split:
                            separator = "|,|"
...
def download_ftp_file(data_set):
    ...
    try:
        ...
        if ftp_usr == '' or ftp_srv == '' or ftp_pass == '' or file_to_download == '':
            CommonUtil.ExecLog(
                sModuleInfo, "FTP server info not given properly, please see action help", 3)
            return "failed"
        ...
        if local_file_path == '':
            ...
            CommonUtil.ExecLog(
                sModuleInfo, "Local file path not given, downloading the ftp file in the root directory path '%s'" % str(local_file_path), 2)
...
def write_into_single_cell_in_excel(data_set):
    ...
    try:
        ...
        if sheet_name == '' or colmn == '' or colmun_number == '' or excel_file_path == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Excel file info not given properly, please see action help", 3)
            return "failed"
...
def save_text_from_file_into_variable(data_set):
    ...
    try:
        ...
        if text_file_path == '' or var_name == '':
            CommonUtil.ExecLog(
                sModuleInfo, "Text file info not given properly, please see action help", 3)
            return "failed"
        ...
        CommonUtil.ExecLog(sModuleInfo, "Text %s is found inside text file %s" % (
            var_value.strip(), text_file_path), 1)
        CommonUtil.ExecLog(sModuleInfo, "Saving text %s into variable named %s" % (
            var_value.strip(), var_name), 1)

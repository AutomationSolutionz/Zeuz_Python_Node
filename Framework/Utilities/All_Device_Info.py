...
def get_all_connected_android_info():
    ...
    try:
        ...
        # Usually needed for CDMA phones
        android_cmd_alt_imei = "shell service call iphonesubinfo 1"
        ...
        for serial in android_list:
            ...
            os_version = subprocess.check_output(
                'adb -s %s %s' % (serial, android_cmd_os_version), shell=True)
            model = subprocess.check_output(
                'adb -s %s %s' % (serial, android_cmd_model), shell=True)
            name = subprocess.check_output(
                'adb -s  %s %s' % (serial, android_cmd_name), shell=True)
            mfg = subprocess.check_output(
                'adb -s %s %s' % (serial, android_cmd_mfg), shell=True)
            imei = subprocess.check_output(
                'adb -s %s %s' % (serial, android_cmd_imei), shell=True)
            ...
            imei = imei.strip()
            if imei != '':
                # Last line should be IMEI
                imei = str(imei.split('\n')[-1]).strip()
                # Last word on this line is IMEI
                imei = str(imei.split(' ')[-1]).strip()
            else:  # Try alternative method to get imei
                imei = subprocess.check_output(
                    'adb -s %s %s' % (serial, android_cmd_alt_imei), shell=True)
                imei = imei.split(' ')
                final = ''
                for line in imei:
                    if "'" in line:
                ...
                imei = final.replace('\r', '').replace('\n', '').replace("'", '').replace(
                    '.', '').replace(')', '')  # Remove all unnecessary characters
    ...
    try:
        ...
        for uuid in ios_list:
            ...
            try:
                ...
                try:
                    device_list[dname]['imei'] = info_list['InternationalMobileEquipmentIdentity']
                except:
...
def get_all_booted_ios_simulator_info():
    ...
    try:
        ...
        all_ios_simulators = subprocess.check_output(
            'xcrun simctl list --json', shell=True)
...
def get_all_connected_device_info():
    ...
    try:
        ...
        if sys.platform == 'darwin':
            ...
            device_list.update(get_all_connected_ios_info())

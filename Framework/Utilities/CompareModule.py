...
class CompareModule():
    def compare(self, expected_list, actual_list, keywordlist=[], ignorelist=[]):
        ...
        if len(keywordlist) == 0:
            ...
            data_return_expected = eliminate_duplicate(
                expected_copy, keywordlist)
            expected_copy = copy.deepcopy(data_return_expected['valid'])
            duplicate_expected = copy.deepcopy(
                data_return_expected['duplicate'])
            ...
            status = []
            tag = "Matching"
            CommonUtil.ExecLog(sModuleInfo, "%s Datasets: %d" %
                               (tag, len(matching_list)), status_flag)
            for i, each in enumerate(matching_list):
                CommonUtil.ExecLog(sModuleInfo, "%s Dataset: #%d" %
                                   (tag, (i+1)), status_flag)
                expected_list = each[0]
                actual_list = each[1]
                status.append(single_dataset_compare(
            ...
            log_to_db(sModuleInfo, non_key_field_expected,
                      'KeyField Missing', status_flag)
            log_to_db(sModuleInfo, non_key_field_actual,
                      'KeyField Extra', status_flag)
            log_to_db(sModuleInfo, expected_duplicate,
                      'Duplicate in Expected', status_flag)
            log_to_db(sModuleInfo, actual_duplicate,
                      'Duplicate in Actual', status_flag)
            if len(dataset_without_keyfield_actual) > 0 or len(dataset_without_keyfield_expected) > 0 or len(missing_datasets) > 0 or len(extra_datasets) > 0:
                return "Failed"
            else:
                tag = True
...
def log_to_db(sModuleInfo, datasets, tag, status):
    ...
    for i, eachdataset in enumerate(datasets):
        ...
        CommonUtil.ExecLog(sModuleInfo, "%s Tuple Data Entry Count: %d" % (
            tag, len(tuple_data)), status)
        for j, eachitem in enumerate(tuple_data):
            CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s" %
                               ((j+1), eachitem[0], eachitem[1]), status)
        for j, eachitem in enumerate(eachdataset):
            if isinstance(eachitem[1], list):
                CommonUtil.ExecLog(sModuleInfo, "%s Group Data Label: %s" % (
                    tag, eachitem[0]), status)
                CommonUtil.ExecLog(sModuleInfo, "%s Group Data Entry Count: %d" % (
                    tag, len(eachitem[1])), status)
                for k, eachdata in enumerate(eachitem[1]):
                    CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s" % (
                        (k+1), eachdata[0], eachdata[1]), status)
...
def single_dataset_compare(expected_copy, actual_copy):
    ...
    for index in range(len(expected_list)-1, -1, -1):
        ...
        if element[4]:
            ...
            if element[1] == '':
                expected_tuple_data.append(
                    (element[0], element[1], element[2]))
            else:
                expected_group_data.append(
                    (element[0], element[1], element[2]))
    actual_tuple_data = []
    actual_group_data = []
    ...
    matching_tuple_data = list(
        set(expected_tuple_data) & set(actual_tuple_data))
    missing_tuple_data = list(
        set(expected_tuple_data) - set(actual_tuple_data))
    extra_tuple_data = list(set(actual_tuple_data) - set(expected_tuple_data))

    matching_group_data = list(
        set(expected_group_data) & set(actual_group_data))
    missing_group_data = list(
        set(expected_group_data) - set(actual_group_data))
    extra_group_data = list(set(actual_group_data) - set(expected_group_data))

    matching_group_record_label = list(
        set([x[0] for x in matching_group_data]))
    final = []
    ...
    final_missing_group_data = filter(
        lambda x: len(x[1]) > 0, final_missing_group_data)
    final_extra_group_data = filter(
        lambda x: len(x[1]) > 0, final_extra_group_data)
    if len(missing_tuple_data) > 0 or len(extra_tuple_data) > 0 or len(final_missing_group_data) > 0 or len(final_extra_group_data) or len(group_data_not_matching) > 0:
        ...
        status = 1
    CommonUtil.ExecLog(sModuleInfo, "Matching Records: %d" %
                       len(matching_tuple_data), status)
    for i, each in enumerate(matching_tuple_data):
        CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s : %s" %
                           ((i+1), each[0], each[2], each[2]), status)

    CommonUtil.ExecLog(sModuleInfo, "Missing Records: %d" %
                       len(missing_tuple_data), status)
    for i, each in enumerate(missing_tuple_data):
        ...
        for j, eachitem in enumerate(extra_tuple_data):
            if((element[0], element[1]) == (eachitem[0], eachitem[1])):
                CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s : %s" % (
                    (j+1), element[0], element[2], eachitem[2]), status)
                extra_tuple_data.pop(j)
                found = True
                break
        if not found:
            CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s : %s" % (
                (i+1), element[0], element[2], 'N/A'), status)
    CommonUtil.ExecLog(sModuleInfo, "Extra Records: %d" %
                       len(extra_tuple_data), status)
    for i, each in enumerate(extra_tuple_data):
        CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s : %s" %
                           ((i+1), each[0], 'N/A', each[2]), status)

    CommonUtil.ExecLog(sModuleInfo, "Matching Group Records: %d" %
                       len(matching_group_data), status)
    for i, each in enumerate(matching_group_data):
        CommonUtil.ExecLog(
            sModuleInfo, "Matching Group Record: #%d" % (i+1), status)
        CommonUtil.ExecLog(
            sModuleInfo, "Matching Group Label: %s" % each[0], status)
        CommonUtil.ExecLog(
            sModuleInfo, "Matching Group Entry Count: %d" % len(each[1]), status)
        for j, eachitem in enumerate(each[1]):
            CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s : %s" % (
                (j+1), eachitem[0], eachitem[1], eachitem[1]), status)
    final_list = []
    ...
    CommonUtil.ExecLog(sModuleInfo, "Non Match Group Data: %d" %
                       len(final_list), status)
    for each in final_list:
        CommonUtil.ExecLog(
            sModuleInfo, "Non Match Group Label: %s" % each[0], status)
        CommonUtil.ExecLog(
            sModuleInfo, "Non Match Group Entry Count: %d" % len(each[1]), status)
        for i, eachitem in enumerate(each[1]):
            CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s :%s" % (
                (i+1), eachitem[0], eachitem[1], eachitem[2]), status)
    CommonUtil.ExecLog(sModuleInfo, "Missing Group Data: %d" %
                       len(final_missing_group_data), status)
    for each in final_missing_group_data:
        CommonUtil.ExecLog(
            sModuleInfo, "Missing Group Label: %s" % each[0], status)
        CommonUtil.ExecLog(
            sModuleInfo, "Missing Group Entry Count: %d" % len(each[1]), status)
        for i, eachitem in enumerate(each[1]):
            CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s : %s" % (
                (i+1), eachitem[0], eachitem[1], 'N/A'), status)
    CommonUtil.ExecLog(sModuleInfo, "Extra Group Data: %d" %
                       len(final_extra_group_data), status)
    for each in final_extra_group_data:
        CommonUtil.ExecLog(
            sModuleInfo, "Extra Group Label: %s" % each[0], status)
        CommonUtil.ExecLog(
            sModuleInfo, "Extra Group Entry Count: %d" % len(each[1]), status)
        for i, eachitem in enumerate(each[1]):
            CommonUtil.ExecLog(sModuleInfo, "#%d : %s : %s : %s" % (
                (i+1), eachitem[0], 'N/A', eachitem[1]), status)

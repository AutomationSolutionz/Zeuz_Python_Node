...
def RunProcess(sTesterid):
    ...
    while (1):
        try:
            ...
            status = DB.GetData(
                conn,
                "Select status from test_run_env where tester_id = '%s' and status in ('Submitted','Unassigned') limit 1 " %
                (sTesterid))
            conn.close()
            if len(status) == 0:
                continue
            if status[0] != "Unassigned":
                if status[0] == "Submitted":
                    ...
                    current_path = os.path.join(
                        FileUtilities.get_home_folder(), os.path.join(
                            'Desktop', 'AutomationLog'))
                    retVal = FileUtilities.CreateFolder(
                        current_path, forced=False)
                    if retVal:
                        # now save it in the global_config.ini
                        TEMP_TAG = 'Temp'
                        file_name = ConfigModule.get_config_value(
                            TEMP_TAG, '_file')
                        current_path_file = os.path.join(
                            current_path, file_name)
                        FileUtilities.CreateFile(current_path_file)
                        ConfigModule.clean_config_file(current_path_file)
                        ConfigModule.add_section(
                            'sectionOne', current_path_file)
                ...
                DB.UpdateRecordInTable(
                    conn, "test_run_env", "where tester_id = '%s' and status = 'Unassigned'" %
                    sTesterid, last_updated_time=last_updated_time)
                conn.close()
...
def Login():
    try:
        ...
        result = Check_Credentials(
            username, password, project, team, server, port)
        if result:
            tester_id = update_machine(
...
def update_machine(dependency):
    try:
        ...
        DATABASE_TAG = 'Database'
        server = ConfigModule.get_config_value(
            DATABASE_TAG, 'database_address')
        ...
        if isinstance(count, list) and count[0] == 0:
            ...
            result = DB.InsertNewRecordInToTable(
                Conn, "permitted_user_list", **temp_Dict)
            Conn.close()
        ...
        status = DB.GetData(
            conn,
            "Select status from test_run_env where tester_id = '%s'" %
            testerid)
        conn.close()
        ...
        if result:
            conn = DB.ConnectToDataBase(sHost=server)
            machine_id = DB.GetData(
                conn,
                "select id from test_run_env where tester_id='%s' and status='Unassigned'" %
                testerid)
            conn.close()
            ...
            if isinstance(project_id, list) and len(project_id):
                ...
                project_id = ''
            conn = DB.ConnectToDataBase(sHost=server)
            teamValue = DB.GetData(
                conn, "select id from team where team_name='%s' and project_id='%s'" %
                (team, project_id))
            conn.close()
            ...
            conn = DB.ConnectToDataBase(sHost=server)
            result = DB.InsertNewRecordInToTable(
                conn, "machine_project_map", **temp_dict)
            conn.close()
            if result:
                return testerid
        return False
...
def Check_Credentials(username, password, project, team, server, port):
    ...
    try:
        query = "select distinct user_id::text from user_project_map upm, projects p where upm.project_id=p.project_id and p.project_name='%s'" % (
            project.strip())
        Conn = DB.ConnectToDataBase(sHost=server)
        ...
        # print message
        query = "select count(*) from user_info ui, permitted_user_list pul where ui.full_name=pul.user_names and username='%s' and password='%s' and user_level not in ('email','Automation', 'Manual') and user_id in (%s)" % (
            username, base64.b64encode(password), message)
        Conn = DB.ConnectToDataBase(sHost=server)
        count = DB.GetData(Conn, query)
        Conn.close()
        # print count
...
def dependency_collection(project, team, server):
    try:
        ...
        if isinstance(project_id, list) and len(project_id) == 1:
            ...
            project_id = ''
        query = "select distinct dependency_name from dependency d ,dependency_management dm where d.id=dm.dependency and dm.project_id='%s' and dm.team_id=(select id from team where team_name='%s' and project_id='%s')" % (

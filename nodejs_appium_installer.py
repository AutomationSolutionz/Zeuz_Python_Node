...
def install_appium():
    ...
    subprocess.run([str(npm_path), "install", "-g", "appium-uiautomator2-driver"], check=True)
    subprocess.run([str(npm_path), "install", "-g", "appium-xcuitest-driver"], check=True)
...
def check_installations():
    ...
    if npm_path.exists():
        try:
            result = subprocess.run([str(npm_path), "list", "-g", "appium"], 

...
// Seems unnecessary code in the context of selenium-webdriver actions
...
// This function wont be needed, if Click action is used instead of select-action
...
/*
    Event handler codes that implements both codeGen logics and Zeuz Logics.
    Still incomplete. Need to work on scroll, drag-n-drop, swtich tab, open tab, switch iframe, etc
*/

const isRangeInput = function(target) {
    if (!target || target.nodeName !== 'INPUT')
        return false;
    return target.type.toLowerCase() === 'range';
}
const asCheckbox = function(target) {
    if (!target || target.nodeName !== 'INPUT')
        return null;
...
const _shouldGenerateKeyPressFor = function(event) {
...
const onInput = function(event) {
    ...
    if (target.nodeName === 'SELECT') {
        ...
        recorder.record({
            name: 'select', // in back_recorder.js it is converted to 'click'
            xpath: recorder.getXpath([...selectElement.selectedOptions][0]),
            value: '',
        });
    }
}
    ...
    if (event.key === ' ') {
        ...
        if (checkbox) {
            recorder.record({
                name: 'check uncheck',
                xpath: recorder.getXpath(target),
                value: checkbox.checked ? 'uncheck' : 'check',
            });
            return;
        }
    }

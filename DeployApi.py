...
def form_uri(resource_path):
    ...
    base_server_address = 'http://%s:%s/' % (
        str(web_server_address), str(web_server_port))
    return base_server_address+resource_path+'/'


def Get(resource_path, payload={}):
    try:
        req = requests.get(form_uri(resource_path),
                           params=json.dumps(payload), timeout=100000).json()
        return req
    except:
        return {}
...
def get_run_time_params_from_user():
    ...
    for key in run_time_params:
        print "******************** SELECT RUN TIME PARAMETER %s ********************" % (
            key.upper())
        i = 0
        while i < len(run_time_params[key]):
            print '%d. %s( %s )' % (
                i + 1, run_time_params[key][i][0], run_time_params[key][i][1])
            i += 1
        print '%d. NONE' % (i + 1)
        print "ENTER YOUR CHOICE: "
        choice = int(str(raw_input()).strip())
        if choice == i+1:  # NONE
            continue
        selected_run_param.append('%s|%s|%s' % (
            key, run_time_params[key][choice-1][0], run_time_params[key][choice-1][1]))
                ...
                if set == '':
                    ...
                    if result == {}:
                        ...
                        if loop == 1:
                            ...
                            print "Result of Loop %d : " % (
                                i+1) + str(converted_to_string_dict)
            if delete_runid:
